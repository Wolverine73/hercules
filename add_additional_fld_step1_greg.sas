...
data data_pnd.t_&initiative_id._1_2_grg;
  set data_pnd.t_&initiative_id._1_2_bkp;
run;
...
%let CLIENT_ID_CONDITION = %str(AND A.CLIENT_ID  IN (18434));
...
proc sql;
  drop table &DB2_TMP..i&INITIATIVE_ID._FILL_DT;
quit;

 PROC SQL;
   CONNECT TO DB2 AS DB2(DSN=&UDBSPRP);

     EXECUTE(CREATE TABLE &DB2_TMP..i&INITIATIVE_ID._FILL_DT
...
PROC SQL;
   ...
   EXECUTE
  (ALTER TABLE &DB2_TMP..i&INITIATIVE_ID._FILL_DT ACTIVATE NOT LOGGED INITIALLY  ) BY DB2;
   EXECUTE(INSERT INTO &DB2_TMP..i&INITIATIVE_ID._FILL_DT
           SELECT    
...
/*    		 max(A.RX_NB)        AS RX_NB,*/
        ...
        FROM     &CLAIMSA..&CLAIM_HIS_TBL               AS A,
                 ...
                 &CLAIMSA..TDRUG1                       AS E,
/*                 DATA_PND.T_&INITIATIVE_ID._1_2         AS F,*/
                 &DB2_TMP..&TABLE_PREFIX.PT_DRUG_NM_QL  AS G

        WHERE    A.PT_BENEFICIARY_ID=G.PT_BENEFICIARY_ID
/*          AND    G.LAST_FILL_DT = A.FILL_DT*/
...
proc sql;
select count(*)
from &DB2_TMP..i&INITIATIVE_ID._FILL_DT;
quit;
