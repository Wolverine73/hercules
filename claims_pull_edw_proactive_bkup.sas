...
/*               AND CLT.CLIENT_LEVEL_1<>'GYY'*/
  ...
  *SASDOC--------------------------------------------------------------------------
  |04feb2013 - Hercules Stabilization
  |Extract Practioner and join to claim_drug
  +------------------------------------------------------------------------SASDOC*;
  		PROC SQL;
  			CREATE TABLE claim_drug_practitioner AS
  		 SELECT c.*,
				prctr.prctr_id AS practitioner_id,
                prctr.ql_prscr_id AS prescriber_id, 
                prctr.entity_ind,
                prctr.degr_1_cd
              FROM claim_drug c left join &DSS_CLIN..V_PRCTR_DENORM PRCTR
  ...
  +------------------------------------------------------------------------SASDOC*;
      ...
      proc sql;
       create table claims_phmcy as
        select *
          from claim_drug_practitioner c,
               phmcy p
          where c.phmcy_gid = p.phmcy_gid
  		;quit;
  *SASDOC--------------------------------------------------------------------------
  |08NOV2012 - Hercules STabilization 
  |Extract Client Specific MOR claims and delete them from claims_phcmy
  +------------------------------------------------------------------------SASDOC*;
  *SASDOC--------------------------------------------------------------------------
  |RXCLAIMS
  +------------------------------------------------------------------------SASDOC*;
...
%END;
  ...
  *SASDOC--------------------------------------------------------------------------
  ...
  |13JUN2012: Apply business rule 2 here for including MOR Claim overrides - AK 
  +------------------------------------------------------------------------SASDOC*;

      proc sql;
       create table morclaims as
        select p.NABP_CODE_6,mc.mbr_gid,mc.client_level_1,mc.client_level_2,mc.client_level_3
          from mail_claims mc,
               phmcy p
          where mc.phmcy_gid = p.phmcy_gid
            AND p.NABP_CODE_6 IN ('482663','146603','032664','012929','398095','459822','032691','147389','458303','100229')
          ;
      quit;
...
proc sql;
create table data_pnd.morclaims_overrides as
select a.* from morclaims a, &rule_table. b
WHERE b.BUS_RULE_TYPE_CD  = 2 AND B.PROGRAM_ID = 72 AND &HIERARCHY_CONS.  ;
quit;


DATA DATA_PND.MORCLAIMS_&ADJ_ENGINE.;
SET DATA_PND.MORCLAIMS_OVERRIDES;
RUN;

DATA DATA_PND.MORCLAIMS_OVERRIDES_&ADJ_ENGINE.;
SET DATA_PND.MORCLAIMS_OVERRIDES;
RUN;
...
%if %sysfunc(exist(morclaims)) %then %do;
proc sql;
delete from morclaims a
where a.mbr_gid not in (select distinct b.mbr_gid
from data_pnd.morclaims_overrides b) ;
quit;
%end;
...
/*	END AK ADDED CODE	*/
      ...
      proc sql;
        DELETE FROM claims_phmcy A WHERE a.mbr_gid in  ( SELECT b.mbr_gid FROM morclaims B WHERE A.mbr_gid = B.mbr_gid ) ;
            ;
      QUIT;
