...
%*SASDOC -----------------------------------------------------------------------
 ...
 +-------------------------------------------------------------------------SASDOC;
		...
		PROC SQL NOPRINT;
        	CONNECT TO DB2 AS DB2(DSN=&UDBSPRP.);
			CREATE TABLE CPG_HIERARCHIES AS
...
%*SASDOC -----------------------------------------------------------------------
 ...
 +-------------------------------------------------------------------------SASDOC;
		...
		PROC SQL;
		 CREATE TABLE CPG_HIERARCHIES AS
		 SELECT A.*, B.PLAN_NM
		 FROM CPG_HIERARCHIES (DROP=PLAN_NM) A
		 LEFT JOIN
		      PLAN_NM B
...
%*SASDOC ----------------------------------------------------------------------------
 ...
 +----------------------------------------------------------------------------SASDOC*;
		...
		PROC SQL NOPRINT;
			CREATE TABLE CLIENT_CPG_LIST AS
...
%*SASDOC ----------------------------------------------------------------------------
 ...
 +----------------------------------------------------------------------------SASDOC*;
		...
		PROC SORT DATA = CLIENT_CPG_LIST; 
...
/*			QUIT;*/
			...
			PROC SQL NOPRINT;
				CREATE TABLE CPG_HIERARCHIES2 AS
      			SELECT  DISTINCT A.CLIENT_ID, A.CLT_PLAN_GROUP_ID, A.CLT_SETUP_DEF_CD
		 		FROM  CPG_HIERARCHIES AS A
                WHERE A.CLT_SETUP_DEF_CD = 2 OR A.CLT_SETUP_DEF_CD IS NULL;
			QUIT;
            ...
            PROC SQL;
				CREATE TABLE CPG_INCEXC2 AS
				SELECT B.CLIENT_ID, B.CLT_PLAN_GROUP_ID
		        FROM CPG_INCEXC B
                WHERE B.CLT_SETUP_DEF_CD = 2;
			QUIT;

			DATA CLIENT_CPG_LIST_TMP;
				MERGE CPG_HIERARCHIES(WHERE=(CLT_SETUP_DEF_CD = 2 OR CLT_SETUP_DEF_CD IS NULL) IN=A)
				      CPG_INCEXC(WHERE=(CLT_SETUP_DEF_CD = 2) IN=B);
			    BY CLIENT_ID CLT_PLAN_GROUP_ID;
...
%*SASDOC ----------------------------------------------------------------------------
 ...
 +----------------------------------------------------------------------------SASDOC*;
		...
		PROC SORT DATA = CLIENT_CPG_LIST OUT=DATA_PND.CLIENT_CPG_LIST_&SYSJOBID.; 
			BY CLIENT_ID CLT_PLAN_GROUP_ID;
		RUN;
...
%*SASDOC -----------------------------------------------------------------------
 ...
 +-------------------------------------------------------------------------SASDOC;
		...
		%IF &DFL_CLT_INC_EXU_IN. = 1 %THEN %DO;
			...
			DATA DATA_PND.CLIENT_CPG_LIST2_&SYSJOBID.;
				MERGE CPG_HIERARCHIES (IN=A)
			      	  DATA_PND.CLIENT_CPG_LIST_&SYSJOBID. (IN=B);
				BY CLIENT_ID CLT_PLAN_GROUP_ID;
				IF A = 1 AND B = 0 THEN OUTPUT DATA_PND.CLIENT_CPG_LIST2_&SYSJOBID.;
			RUN;
...
%*SASDOC -----------------------------------------------------------------------
 ...
 +-------------------------------------------------------------------------SASDOC;
		...
		PROC SQL NOPRINT;
			...
			FROM  DATA_PND.CLIENT_CPG_LIST_&SYSJOBID.
        	&WHERE_CONS.;
		QUIT;
		%END;
		%ELSE %DO;
		PROC SQL NOPRINT;
			SELECT DISTINCT CLIENT_ID
            INTO :RESOLVE_CLIENT_IDS SEPARATED BY ' , '
			FROM  DATA_PND.CLIENT_CPG_LIST2_&SYSJOBID.
        	&WHERE_CONS.;
		QUIT;
		%END;
...
/*				SET DATA_PND.CLIENT_CPG_LIST2_&SYSJOBID. (KEEP = CLIENT_ID CLT_PLAN_GROUP_ID );*/
/*			RUN;*/
...
/*			PROC SQL; */
/*				DROP TABLE DATA_PND.CLIENT_CPG_LIST_&SYSJOBID.;*/
/*			QUIT;*/
...
/*			PROC SQL; */
/*				DROP TABLE DATA_PND.CLIENT_CPG_LIST2_&SYSJOBID.;*/
/*			QUIT;*/
...
%*SASDOC ----------------------------------------------------------------------------
 ...
 +----------------------------------------------------------------------------SASDOC*;
	...
	PROC SQL NOPRINT;
		CREATE TABLE DATA_PND.CLIENT_CPG_LIST_&SYSJOBID. AS
      	SELECT  DISTINCT A.CLIENT_ID, A.CLT_PLAN_GROUP_ID
		 FROM  CPG_HIERARCHIES AS A
		 WHERE A.CLT_SETUP_DEF_CD = 1;
	QUIT;
...
%*SASDOC ----------------------------------------------------------------------------
 ...
 +----------------------------------------------------------------------------SASDOC*;
	...
	PROC SQL NOPRINT;
		INSERT INTO DATA_PND.CLIENT_CPG_LIST_&SYSJOBID.
      	SELECT  DISTINCT A.CLIENT_ID, A.CLT_PLAN_GROUP_ID
		 FROM  CPG_HIERARCHIES AS A
...
/*		SET DATA_PND.CLIENT_CPG_LIST_&SYSJOBID. (KEEP = CLIENT_ID CLT_PLAN_GROUP_ID );*/
/*	RUN;*/
