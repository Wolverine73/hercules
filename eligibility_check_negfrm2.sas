...
%MACRO ELIGIBILITY_CHECK_NEGFRM2  (tbl_name_in=, 
                                   tbl_name_in_rx=, 
                                   tbl_name_in_re=,
                                   tbl_name_out=, 
                                   tbl_name_rx_out=, 
...
%IF &ELIG_CD=1 or &ELIG_CD=2 %THEN %DO; 
/*logic added by Radhika.Gadde on 10/19/2012 for testing purpose*/
...
%MACRO EDWCHKELIG2(adj=,tbl_name_in_edw=,tbl_name_out_edw=,tbl_name_out2_edw=,adj2=,hier_cd=) ; 
	...
	*SASDOC----------------------------------------------------------------------
	...
	+---------------------------------------------------------------------SASDOC*;
    %if &rx_adj eq 1 AND &adj='X' %then %do;  /** begin rxclaim **/
	   proc sql;
        ...
        SELECT * FROM CONNECTION TO ORACLE
	      (
	        ...
	        where  B.ALGN_GRP_STUS_CD = 'A'
			  ...
			  and  B.ALGN_LVL_GID = A.ALGN_LVL_GID_KEY  
...
/*			EXECUTE (*/
		...
		SELECT * FROM CONNECTION TO ORACLE
	      (
		    select   
        %BQUOTE(/)%BQUOTE(*)+ ordered  use_nl(c) index(c x_mbr_elig_n10) %BQUOTE(*)%BQUOTE(/)
					distinct
	                 A.mbr_id, 
		 	         A.payer_id 
		    from     &tbl_name_in_edw.   A, 
	        ...
	        where   
	               C.ELIG_REC_STUS_CD = 'A' 
			  and  C.SRC_SYS_CD = &ADJ
	          and  C.ELIG_EFF_DT <= to_date(&chk_dt_oracle,'yyyy-mm-dd') 
	          and  C.ELIG_END_DT >= to_date(&chk_dt_oracle,'yyyy-mm-dd')  
	          and  A.MBR_GID  = C.MBR_GID 
	          and  A.PAYER_ID = C.PAYER_ID
			  and  A.PAYER_ID <= 100000
/*			  commented per Oracle DBA suggestion*/
...
/*			  and  A.ALGN_LVL_GID_KEY = C.ALGN_LVL_GID */
	   ...
	   QUIT;
    %end;  
    %if &re_adj eq 1 AND &adj='R' %then %do;  /** begin recap **/
	   proc sql;
	        CONNECT TO ORACLE(PATH=&GOLD );
/*			EXECUTE (*/
	        CREATE TABLE &ora_tmp..PTSa&adj2.&INIT_ID AS
	      ...
	      (
	        ...
	        where 
	 			   A.ALGN_LVL_GID_KEY = B.ALGN_LVL_GID
			  and  a.client_level_1=b.ins_code
			  and  a.client_level_2=b.carr_id
			  and  a.client_level_3=b.grp_id
...
%if &rx_adj eq 1 %then %do;
  %EDWCHKELIG2(adj='X',tbl_name_in_edw=&tbl_name_in_rx,tbl_name_out_edw=&tbl_name_rx_out,
              tbl_name_out2_edw=&tbl_name_rx_out2,adj2=RX,hier_cd=Account_Id) ; 
%end;
%if &re_adj eq 1 %then %do;
  %EDWCHKELIG2(adj='R',tbl_name_in_edw=&tbl_name_in_re,tbl_name_out_edw=&tbl_name_re_out,
              tbl_name_out2_edw=&tbl_name_re_out2,adj2=RE,hier_cd=insurance_cd) ; 
%end;

%END; *end of Ignore Elig change by Radhika;
%MEND ELIGIBILITY_CHECK_NEGFRM2;
