 /**HEADER -------------------------------------------------------------------------------------------------
  ...
  |           APR 2008 Suresh	- Hercules Version  2.1.01
 ...
 *SASDOC -----------------------------------------------------------------------
  ...
  %INCLUDE "/PRG/sastest1/adhoc/manual_ins_ovr.sas";
...
*SASDOC -----------------------------------------------------------------------------------
  ...
  +--------------------------------------------------------------------------------SASDOC*;
	...
	PROC SQL;
	 SELECT COUNT(*) INTO :MODMAIL
	 FROM DICTIONARY.COLUMNS
	 WHERE LIBNAME= "DATA_PND" 
	       AND MEMNAME = "&TABLE_PREFIX._&&CMCTN_ROLE_CD&I."
		   AND UPCASE(NAME) = "ADJ_ENGINE";
	QUIT;

	%SET_ERROR_FL;


	%IF &MODMAIL. > 0 %THEN %DO;
	...
	%IF &ADJ_ENGINE = RX %THEN %DO;
		...
		%LET HIERARCHY_OVR_CONS = %STR( B.CARRIER_ID = A.CLIENT_LEVEL_1
            						AND (B.ACCOUNT_ID = ' ' OR 
	...
	%IF &ADJ_ENGINE = RE %THEN %DO;
		...
		%LET HIERARCHY_OVR_CONS = %STR( B.INSURANCE_CD = A.CLIENT_LEVEL_1
            						AND (B.CARRIER_ID = ' ' OR 
	...
	| IF NO ROWS EXISTS IN OVERRIDE TABLES
 	+------------------------------------------------------------------------------SASDOC;
  			...
  			%IF &RCNT_DEF >= 1 AND &RCNT_ADJ = 0 %THEN %DO;
         		...
         		PROC SQL;
         			CREATE TABLE &ADJ_ENGINE._&TABLE_PREFIX._&&CMCTN_ROLE_CD&I. AS
	...
	| IF ROWS EXISTS IN OVERRIDE TABLES
 	+------------------------------------------------------------------------------SASDOC;
			...
			PROC APPEND BASE = DATA_PND.&TABLE_PREFIX._&&CMCTN_ROLE_CD&I.
			            DATA = &ADJ_ENGINE._&TABLE_PREFIX._&&CMCTN_ROLE_CD&I.;
			RUN;
