...
+------------------------------------------------------------------------SASDOC*;
%MACRO CLAIMS_PULL_EDW_AST(DRUG_NDC_TABLE_RX = &ORA_TMP..&TABLE_PREFIX._NDC_RX,
					   DRUG_NDC_TABLE_RE = &ORA_TMP..&TABLE_PREFIX._NDC_RE, 
					   DRUG_RVW_DATES_TABLE = &ORA_TMP..&TABLE_PREFIX._RVW_DATES,
					   RESOLVE_CLIENT_TABLE_RX = &ORA_TMP..&TABLE_PREFIX._RX ,
					   RESOLVE_CLIENT_TABLE_RE = &ORA_TMP..&TABLE_PREFIX._RE
...
%IF &PROGRAM_ID NE 105 %THEN %DO;
		...
		PROC SQL;
			...
			EXECUTE
			(
				CREATE TABLE TMP_CLAIMS_PULL_&INITIATIVE_ID._&ADJ_ENGINE. AS
				SELECT 	/* + ORDERED */
                        ...
                        /*						NEW FIELDS ADDED TO CLAIMS TABLE*/
						...
						/*						NEW FIELDS ADDED TO DRUG*/
						,DRUG.GPI_CODE AS GPI_THERA_CLS_CD
						,CASE 
							WHEN DRUG.RECAP_GNRC_FLAG = '2' OR MULTI_TYPE_CODE IN ('M','O','N')
								THEN 'B' ELSE 'G' 
		...
		QUIT;
				...
				PROC SQL;
		CREATE TABLE TMP_CLAIMS_PULL_&INITIATIVE_ID._&ADJ_ENGINE._2 AS
				SELECT CLAIMS.* ,FRMLY.FRMLY_NB AS FORMULARY_TX
					FROM TMP_CLAIMS_PULL_&INITIATIVE_ID._&ADJ_ENGINE. CLAIMS
					LEFT JOIN
		...
		CREATE TABLE TMP_CLAIMS_PULL_&INITIATIVE_ID._&ADJ_ENGINE. AS
				...
				(SELECT 
					...
					(
						SUBSTR(PRCTR.PRCTR_ID,1,1) NOT IN ('1','2','3','4','5','6','7','8','9','0')
													OR
						SUBSTR(PRCTR.PRCTR_ID,1,2) NOT IN ('1','2','3','4','5','6','7','8','9','0')
					)
		...
		ON  CLAIMS.PRCTR_NPI_ID = PRCTR.PRCTR_NPI_ID;
		QUIT;

/*		/ON  PRCTR.PRCTR_ID_TYP_CD = 'DH' */
/*                AND PRCTR.PRCTR_ID <> PRCTR.PRCTR_NPI_ID */
/*                  the first two digits of the PRCTR_ID not in */
/*                AND SUBSTR(PRCTR.PRCTR_ID,1,2) not in ('1','2','3','4','5','6','7','8','9','0');*/
/*			QUIT;*/
