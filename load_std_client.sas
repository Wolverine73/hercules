...
LIBNAME PBATCH DB2 DSN=&UDBSPRP SCHEMA=PBATCH DEFER=YES;
...
IF GSTP_GSA_PGMTYP_CD  IN (1,2,3) THEN OUTPUT;
run;
...
infile rxfile dlm=',' dsd missover firstobs=2;
input 
PROGRAM_ID                 :4.
TASK_ID                    :2.
CARRIER_ID                 :$20.
...
CREATE TABLE STD_DRUG_CLASS AS
...
, char(A.DRG_CLS_EFF_DT) as DRG_CLS_EFF_DT
, char(A.DRG_CLS_EXP_DT) as DRG_CLS_EXP_DT
					

FROM HERCULES.TGSTP_DRG_CLS A
...
PROC SORT DATA=STD_DRUG_CLASS; BY GSTP_GSA_PGMTYP_CD DRG_CLS_SEQ_NB;
RUN;
...
*SASDOC--------------------------------------------------------------------------
| Insert recaords into HERCULES.TPGMTASK_*_RUL tables
+------------------------------------------------------------------------SASDOC*;
...
,GSTP_GSA_PGMTYP_CD
,input(EFFECTIVE_DT,MMDDYY10.)
,input(DRG_CLS_EFF_DT,YYMMDD10.)
,input(DRG_CLS_EXP_DT,YYMMDD10.)
,'QCPAP020'
,'QCPAP020'
