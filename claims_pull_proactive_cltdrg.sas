...
/*		%PUT NOTE: DS_STRING_RX_RE = &DS_STRING_RX_RE. ;*/
		...
		SELECT * FROM CONNECTION TO ORACLE	
			(
		SELECT 	DISTINCT CLTDRG.PROGRAM_ID, CLTDRG.QL_CLIENT_ID, 
				CLTDRG.CLIENT_LEVEL_1, CLTDRG.CLIENT_LEVEL_2, CLTDRG.CLIENT_LEVEL_3
/*				,CLTDRG.DRUG_GID*/
		FROM &CLIENT_DRUG_TABLE. 
		WHERE SYSDATE BETWEEN EFFECTIVE_DT AND EXPIRATION_DT
...
%local keycol1 keycol2 keycol3;

%if &ADJ_ENGINE=RX %then %do;
  %let KEYCOL1=%str(RSLV.CARRIER_ID as CLIENT_LEVEL_1);
  %let KEYCOL2=%str(RSLV.ACCOUNT_ID as CLIENT_LEVEL_2);
  %let KEYCOL3=%str(RSLV.GROUP_CD as CLIENT_LEVEL_3);
%end;
%else
%if &ADJ_ENGINE=RE %then %do;
  %let KEYCOL1=%str(RSLV.INSURANCE_CD as CLIENT_LEVEL_1);
  %let KEYCOL2=%str(RSLV.CARRIER_ID as CLIENT_LEVEL_2);
  %let KEYCOL3=%str(RSLV.GROUP_CD as CLIENT_LEVEL_3);
%end;
		...
		PROC SQL;
			...
			(
				...
				SELECT 	%BQUOTE(/)%BQUOTE(*)+ ORDERED use_hash(claim) full(claim) %BQUOTE(*)%BQUOTE(/)
						 CLTDRG.ALGN_LVL_GID_KEY
						,CLTDRG.CUST_NM AS CLIENT_NM
						,&KEYCOL1.
						,&KEYCOL2.
						,&KEYCOL3.
						,CLAIM.PAYER_ID
...
/*						&REFILL_QTY.*/
				...
				WHERE   CLAIM.DSPND_DATE BETWEEN &CLM_BEGIN_DT_CONV. AND &CLM_END_DT_CONV. 
					...
					AND CLAIM.PAYER_ID = CLTDRG.PAYER_ID
					AND	CLTDRG.PROGRAM_ID = &PROGRAM_ID.								
					AND CLTDRG.PAYER_ID = RSLV.PAYER_ID
					AND CLTDRG.ALGN_LVL_GID_KEY = RSLV.ALGN_LVL_GID_KEY					
...
/*	Mail claims for the client specific drug setup	*/
PROC SQL;
  			CONNECT TO ORACLE(PATH=&GOLD PRESERVE_COMMENTS);
  			CREATE TABLE data_res.mail_claims_cltdrg_&adj_engine. AS
  				SELECT * FROM CONNECTION TO ORACLE
  				( 
          SELECT  	%bquote(/)%bquote(*) +ORDERED use_hash(claim) full(claim) parallel(claim,16)	%bquote(*)%bquote(/)
...
/*                  claim.QL_BNFCY_ID,*/
              ...
              FROM &DSS_CLIN..V_CLAIM_CORE_PAID CLAIM,
                   &ORA_TMP..CLT_LST_&INITIATIVE_ID._&ADJ_ENGINE. CLT,
				   &ORA_TMP..&TABLE_PREFIX._&ADJ_ENGINE. RSLV

             WHERE 		CLAIM.PAYER_ID IN (SELECT DISTINCT PAYER_ID FROM &ORA_TMP..CLT_LST_&INITIATIVE_ID._&ADJ_ENGINE)
					AND CLAIM.DSPND_DATE BETWEEN &MAIL_BGN_EDW_DT AND &MAIL_END_EDW_DT 
			 		AND CLAIM.PAYER_ID &PAYER_ID_CONS.
					AND CLAIM.BATCH_DATE IS NOT NULL
					AND	CLAIM.SRC_SYS_CD = %bquote('&SRC_SYS_CD')
					AND claim.claim_wshd_cd IN ('P', 'W')
 				    AND CLT.PAYER_ID = RSLV.PAYER_ID
					AND clt.ALGN_LVL_GID_KEY = RSLV.ALGN_LVL_GID_KEY
					AND claim.ALGN_LVL_GID = clt.ALGN_LVL_GID_KEY
				    AND claim.payer_id = clt.payer_id
					AND	CLT.PROGRAM_ID = &PROGRAM_ID.					
         );
          DISCONNECT FROM ORACLE;
  		QUIT;
  /*        		 &MAIL_DELVRY_CD.*/
...
/*						,CAST(DRUG.QL_NHU_TYPE_CD AS INT) AS NHU_TYPE_CD*/
						...
						,DRUG.GPI_CODE AS GPI14
						,CLAIM.REFILL_FILL_QY
						,0 AS LTR_RULE_SEQ_NB
 						,RX_NB
 						,DISPENSED_QY
  						,DAY_SUPPLY_QY /*ADDED DAY_SUPPLY_QY */
						,PHMCY.PHMCY_NAME AS PHARMACY_NM 	           /* NEWLY ADDED COLUMNS */
				...
				WHERE   LTRIM(RTRIM(CLTDRG.CLIENT_LEVEL_1)) = LTRIM(RTRIM(CLAIM.CLIENT_LEVEL_1))
					AND CLAIM.DRUG_GID = CLTDRG.DRUG_GID
					AND	CLTDRG.PROGRAM_ID = &PROGRAM_ID.
					AND  SYSDATE BETWEEN CLTDRG.EFFECTIVE_DT AND CLTDRG.EXPIRATION_DT
...
/*			%PARTICIPANT_EXCLUSIONS(TBL_NAME_IN = &ORA_TMP..TMP3_CLAIMS_PULL_&INITIATIVE_ID._&ADJ_ENGINE.);*/
	...
	%IF &RX_ADJ. = 1 AND %SYSFUNC(EXIST(&CLIENT_DRUG_TABLE_RX.)) AND &ADJ_ENGINE. = RX %THEN %DO;	
										/*	AK ADDED CONDITION ADJ_ENGINE= RX TO CHECK AND RUN FOR THE RX ADJ_ENGINE BASED ON EDW_PROCESS CALL. */ 
...
%PUT &ADJ_ENGINE CLIENT SPECIFIC CLAIMS PULL IS EXECUTING;
		%EDW_CLAIMS(ADJ_ENGINE = RX);
	%END;

	%IF &RE_ADJ. = 1 AND %SYSFUNC(EXIST(&CLIENT_DRUG_TABLE_RE.)) AND &ADJ_ENGINE. = RE %THEN %DO;
										/*	AK ADDED CONDITION ADJ_ENGINE=RE TO CHECK AND RUN FOR THE RE ADJ_ENGINE BASED ON EDW_PROCESS CALL. */ 
