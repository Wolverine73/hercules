...
|
| USAGE:    get_ndc_tbd(DRUG_NDC_TBL=,CLAIM_DATE_TBL=)
|
| Eg. options %get_ndc_tbd(DRUG_NDC_TBL=TMP55.MY_NDC)
|             %get_ndc_tbd(DRUG_NDC_TBL=TMP55.MY_NDC,CLAIM_DATE_TBL=TMP55.MY_DATE)
|
...
|           &CLAIMSA..TDRUG1
|           DSS_CLIN.V_DRUG_DENORM (RECAP AND RXCLAIM)
...
|+-----------------------------------------------------------------------------------------------------------------
| HISTORY: 
| FIRST RELEASE: JUNE 2012 - E BUKOWSKI(SLIOUNKOVA) - CREATED - TARGET BY DRUG/DSA AUTOMATION - COPIED GET_NDC.SAS
| MAIN DIFFERENCE IS TO REFERENCE DSS_CLIN.V_DRUG_DENORM AS A TABLE FOR DRUG DATA SOURCE
|
| DEC 2013 J. Agostinelli BSR - Voided Claims + Alpha GPI fix
+-----------------------------------------------------------------------------------------------------------HEADER*;

%macro get_ndc_tbd(DRUG_NDC_TBL=,
			 DRUG_NDC_TBL_RX=,
			 DRUG_NDC_TBL_RE=, 			
             CLAIM_DATE_TBL=,	   			
...
*SASDOC-----------------------------------------------------------------------
| Default names to the output tables.
| S.Yaramada 30JUL2008
+----------------------------------------------------------------------SASDOC*;
...
*SASDOC-----------------------------------------------------------------------
| In order to minimize the changes to the code, DRUG_NDC_TBL_QL is initialized 
| to the macro variable DRUG_NDC_TBL so that the table naming remains in sync 
| with the existing code for QL adjudication engine.
| S.Yaramada 28JUN2008
+----------------------------------------------------------------------SASDOC*;
...
%LET DRUG_NDC_TBL_EDW_RECAP= &ORA_TMP..&TABLE_PREFIX._NDC_EDW_RECAP;
%LET DRUG_NDC_TBL_EDW_RXCLM= &ORA_TMP..&TABLE_PREFIX._NDC_EDW_RXCLM;
%let DB2_TMP_TBL_EDW_RECAP=&DB2_TMP..&TABLE_PREFIX._&PROG_NAME._RECAP;
%let DB2_TMP_TBL_EDW_RXCLM=&DB2_TMP..&TABLE_PREFIX._&PROG_NAME._RXCLM;
...
%put NOTE: DRUG_NDC_TBL_EDW_RECAP=&DRUG_NDC_TBL_EDW_RECAP;
%put NOTE: DRUG_NDC_TBL_EDW_RXCLM=&DRUG_NDC_TBL_EDW_RXCLM;
%put NOTE: DB2_TMP_TBL_EDW_RECAP=&DB2_TMP_TBL_EDW_RECAP;
%put NOTE: DB2_TMP_TBL_EDW_RXCLM=&DB2_TMP_TBL_EDW_RXCLM;
...
| March 2007 - N.Williams Add logic to If Drug Definition Code eq 5
+----------------------------------------------------------------------SASDOC*;
...
+----------------------------------------------------------------------SASDOC*;
   ...
   quit;
	...
	| For RxClaim and RECAP we need to pull drugs based off DSS_CLIN.V_DRUG_DENORM 
	| Module added by Katya
	+----------------------------------------------------------------------SASDOC*;

	proc sql noprint;
		connect to oracle(PATH=&GOLD);
		CREATE TABLE WORK.ALL_EDW_DRUGS AS
         SELECT * FROM CONNECTION TO ORACLE
         (
         SELECT  DRUG_GID
		        ,NDC_CODE
				,GPI_CODE
				,RECAP_GNRC_CLASS_NBR
				,DRUG_CLASS
				,RX_OTC_IND
				,END_DATE 
				,RECAP_GNRC_FLAG
				,MS_GNRC_FLAG
				,RECAP_MULTI_TYPE_CODE
				,MULTI_TYPE_CODE
				,DRUG_FDB_SRC_FLAG
				,DRUG_MDDB_SRC_FLAG
				,DRUG_RECAP_SRC_FLAG
				,DRUG_RXCLAIM_SRC_FLAG
				,QL_NHU_TYPE_CD
		...
		DATA WORK.ALL_EDW_DRUGS (KEEP=DRUG_GID GPI_GROUP GPI_CLASS GPI_SUBCLASS GPI_NAME GPI_NAME_EXTENSION 
		GPI_FORM GPI_STRENGTH DRUG_NDC_ID DRUG_PRODUCT_ID DGH_GCN_CD RECAP_RX_IN RXCLM_RX_IN RECAP_DRUG_BRAND_CD
		RXCLM_DRUG_BRAND_CD RECAP_GENERIC_AVAIL_IN RXCLM_GENERIC_AVAIL_IN NHU_TYPE_CD)
       ;
		SET WORK.ALL_EDW_DRUGS;
  		GPI_GROUP            = SUBSTR(GPI_CODE,1,2);
  		GPI_CLASS  		     = SUBSTR(GPI_CODE,3,2);
  		GPI_SUBCLASS         = SUBSTR(GPI_CODE,5,2);
  		GPI_NAME             = SUBSTR(GPI_CODE,7,2);
  		GPI_NAME_EXTENSION   = SUBSTR(GPI_CODE,9,2);
  		GPI_FORM             = SUBSTR(GPI_CODE,11,2);
  		GPI_STRENGTH         = SUBSTR(GPI_CODE,13,2);
		DRUG_NDC_ID          = INPUT(NDC_CODE,20.);
		DRUG_PRODUCT_ID      = INPUT(SUBSTR(NDC_CODE,1,9),9.);
		DGH_GCN_CD           = INPUT(RECAP_GNRC_CLASS_NBR,5.);
		NHU_TYPE_CD          = INPUT(QL_NHU_TYPE_CD,4.);

			/* ASSIGN OTC INDICATOR */
		IF (DRUG_MDDB_SRC_FLAG='1' OR DRUG_RXCLAIM_SRC_FLAG='1') 
			THEN DO;
				IF RX_OTC_IND IN ('O','P') THEN RXCLM_RX_IN = 0;
				ELSE IF RX_OTC_IND IN ('R','S') THEN RXCLM_RX_IN = 1;
			END;
		IF (DRUG_FDB_SRC_FLAG='1' OR DRUG_RECAP_SRC_FLAG='1') 
			THEN DO;
				IF DRUG_CLASS = 'O' THEN RECAP_RX_IN = 0;
				ELSE IF DRUG_CLASS = 'F' THEN RECAP_RX_IN = 1;
			END;


		/* ASSIGN BRAND GENERIC INDICATOR */
		IF (DRUG_MDDB_SRC_FLAG='1' OR DRUG_RXCLAIM_SRC_FLAG='1') 
			THEN DO;
				IF MS_GNRC_FLAG =1 THEN RXCLM_DRUG_BRAND_CD = 'G';
				ELSE IF MS_GNRC_FLAG =0 THEN RXCLM_DRUG_BRAND_CD = 'B';
			END;
		IF (DRUG_FDB_SRC_FLAG='1' OR DRUG_RECAP_SRC_FLAG='1') 
			THEN DO;
				IF RECAP_GNRC_FLAG = '1' THEN RECAP_DRUG_BRAND_CD = 'G';
				ELSE IF RECAP_GNRC_FLAG = '2' THEN RECAP_DRUG_BRAND_CD = 'B';
			END;


		
		/* ASSIGN MULTI-SOURCE INDICATOR */
		IF (DRUG_MDDB_SRC_FLAG='1' OR DRUG_RXCLAIM_SRC_FLAG='1') 
			THEN DO;
				IF MULTI_TYPE_CODE IN ('N','M') THEN RXCLM_GENERIC_AVAIL_IN = 0;
				ELSE IF MULTI_TYPE_CODE IN ('O','Y') THEN RXCLM_GENERIC_AVAIL_IN = 1;
			END;
		IF (DRUG_FDB_SRC_FLAG='1' OR DRUG_RECAP_SRC_FLAG='1') 
			THEN DO;
				IF RECAP_MULTI_TYPE_CODE = '2' THEN RECAP_GENERIC_AVAIL_IN = 0;
				ELSE IF RECAP_MULTI_TYPE_CODE = '1' THEN RECAP_GENERIC_AVAIL_IN = 1;
			END;
		RUN;

 %let DB2_TMP_TBL_DRUG_EDW=&DB2_TMP..&TABLE_PREFIX._&PROG_NAME._DENORM;
   %drop_db2_table(tbl_name=&DB2_TMP_TBL_DRUG_EDW);
...
+----------------------------------------------------------------------SASDOC*;
   ...
   execute
   (
   CREATE TABLE &DB2_TMP_TBL_DRUG_EDW
          ( DRUG_GID                INTEGER NOT NULL
           ,GPI_GROUP               CHAR(2)
           ,GPI_CLASS               CHAR(2)
           ,GPI_SUBCLASS            CHAR(2)
           ,GPI_NAME                CHAR(2)
           ...
           ,DRUG_PRODUCT_ID         DECIMAL(9,0)
           ,DGH_GCN_CD              INTEGER
           ,RECAP_RX_IN             SMALLINT
           ,RXCLM_RX_IN             SMALLINT
           ,RECAP_DRUG_BRAND_CD     CHAR(1)
		   ,RXCLM_DRUG_BRAND_CD     CHAR(1)
           ,RECAP_GENERIC_AVAIL_IN  SMALLINT
           ,RXCLM_GENERIC_AVAIL_IN  SMALLINT
           ,NHU_TYPE_CD             SMALLINT)
   NOT LOGGED INITIALLY
   )
   by DB2;
   execute
   (
   ALTER TABLE &DB2_TMP_TBL_DRUG_EDW ACTIVATE NOT LOGGED INITIALLY
   )
   by DB2;
   execute
   (
   ...
   quit;
    ...
    proc sql noprint NOERRORSTOP;
	INSERT INTO &DB2_TMP_TBL_DRUG_EDW
	SELECT DRUG_GID 
           ,GPI_GROUP 
           ,GPI_CLASS 
           ,GPI_SUBCLASS 
           ...
           ,RECAP_DRUG_BRAND_CD
		   ,RXCLM_DRUG_BRAND_CD 
           ,RECAP_GENERIC_AVAIL_IN 
           ,RXCLM_GENERIC_AVAIL_IN 
           ,NHU_TYPE_CD
	FROM WORK.ALL_EDW_DRUGS
	;
	QUIT;
   ...
   %do;
      ...
      FROM
         ...
         SELECT  A.DRG_GROUP_SEQ_NB
				...
				,B.INCLUDE_IN
                ,B.DRUG_ID_TYPE_CD
/* DEC 2013 BSR Voided Claims - Alpha GPI fix */
                ,CAST(B.DRUG_ID AS CHAR(14))         AS DRUG_PRODUCT_NDC_ID
...
/* if target drug type is GCN. */
 ...
 %Macro generate_edw_drug_tbl(platform=);


	
   %drop_db2_table(tbl_name=&&DB2_TMP_TBL_EDW_&PLATFORM.);
 
%*SASDOC-----------------------------------------------------------------------
| JUNE 2012 - TARGET BY DRUG/ DSA AUTOMATION - E BUKOWSKI
| Create a temporary table.
+----------------------------------------------------------------------SASDOC*;
   proc sql noprint;
   connect to DB2 as DB2(dsn=&UDBSPRP AUTOCOMMIT=NO);
   execute
   (
   CREATE TABLE &&DB2_TMP_TBL_EDW_&PLATFORM.
          ( DRG_GROUP_SEQ_NB         SMALLINT NOT NULL
           ,DRG_SUB_GRP_SEQ_NB       SMALLINT NOT NULL
           ,CLAIM_BEGIN_DT           DATE
           ,CLAIM_END_DT             DATE
   ...
   execute
   (
   ALTER TABLE &&DB2_TMP_TBL_EDW_&PLATFORM. ACTIVATE NOT LOGGED INITIALLY
   )
   by DB2;
   execute
   (
   ...
   quit;
      ...
      execute
      (
      INSERT INTO &&DB2_TMP_TBL_EDW_&PLATFORM.
            ( DRG_GROUP_SEQ_NB
             ,DRG_SUB_GRP_SEQ_NB
             ,CLAIM_BEGIN_DT
             ,CLAIM_END_DT
...
/*	if all drug indicator with restrictions. */
			 ...
			 (     A.ALL_DRUG_IN = 1 
			   AND (   COALESCE(A.RX_IN,A.GENERIC_AVAIL_IN) IS NOT NULL 
			   		OR A.DRUG_BRAND_CD IS NOT NULL OR &DRUG_FIELDS_FLAG.=1)
			   AND (     A.RX_IN 			IS NULL OR 
			             A.RX_IN			=B.&PLATFORM._RX_IN)
			   AND (   A.DRUG_BRAND_CD	 IS NULL 
     				OR A.DRUG_BRAND_CD	=B.&PLATFORM._DRUG_BRAND_CD)
			  AND (   A.GENERIC_AVAIL_IN	 IS NULL 
     				OR A.GENERIC_AVAIL_IN	=B.&PLATFORM._GENERIC_AVAIL_IN) 
			 )
...
/* if target drug type is NDC-11. */
         ...
         OR (   (    A.DRUG_ID_TYPE_CD     = 3)
             AND(    A.DRUG_PRODUCT_NDC_ID =B.DRUG_NDC_ID)
             AND(    A.NHU_TYPE_CD IS NULL OR B.NHU_TYPE_CD IS NULL
                 OR  A.NHU_TYPE_CD =  	B.NHU_TYPE_CD  )
			 )
...
/*             OR (B.DISCONTINUANCE_DT IS NULL))*/
      ...
      disconnect from DB2;
      quit;
	  %MEND generate_edw_drug_tbl;

	%generate_edw_drug_tbl(platform=RXCLM);
	%generate_edw_drug_tbl(platform=RECAP);

	%*SASDOC-----------------------------------------------------------------------
	| What we got inclusion/exclusion of drugs. COULD INCLUDE THIS CODe IN THE QUERY 
	| IF U WANTED JUST INCLUSION.
	+----------------------------------------------------------------------SASDOC*;
...
*SASDOC--------------------------------------------------------------------------
|JUNE 2012 - TARGET BY DRUG/ DSA AUTOMATION
|EB - REMOVE EXCLUDED DRUGS FOR RECAP AND RXCLAIM 
+------------------------------------------------------------------------SASDOC*;
%macro remove_excluded_drugs(platform=);

PROC SQL NOPRINT;
SELECT DRUG_NDC_ID into: DRUG_NDC_ID_&PLATFORM. separated by ',' FROM &&DB2_TMP_TBL_EDW_&PLATFORM.
WHERE INCLUDE_IN = 0
AND DRUG_NDC_ID IS NOT NULL;
QUIT;

%PUT NOTE: NDCS_EXCLUDED_&PLATFORM. = &&DRUG_NDC_ID_&PLATFORM.;

PROC SQL NOPRINT;
CONNECT TO DB2 AS DB2(DSN=&UDBSPRP);
EXECUTE(DELETE FROM &&DB2_TMP_TBL_EDW_&PLATFORM. 
WHERE DRUG_NDC_ID IN (&&DRUG_NDC_ID_&PLATFORM.)
OR INCLUDE_IN = 0) BY DB2;
QUIT;

%mend remove_excluded_drugs;

%remove_excluded_drugs(platform=RXCLM);
%remove_excluded_drugs(platform=RECAP);
...
*SASDOC--------------------------------------------------------------------------
|JUNE 2012 - TARGET BY DRUG/ DSA AUTOMATION
|EB - HANDLE EXCLUDED DRUGS FOR RECAP AND RXCLAIM 
+------------------------------------------------------------------------SASDOC*;
	  	  %macro handle_excluded_drugs(platform=);

	  		%*SASDOC-----------------------------------------------------------------------
		|  Pull All Drugs then Check what has been setup inclusion/exculsion based 
		|  to decide what to place in the output table.  N. Williams 30MAR2007
		+----------------------------------------------------------------------SASDOC*;
		   %let DB2_TMP_TBL3=&DB2_TMP..&TABLE_PREFIX._ALL_NDC_&PLATFORM;
		   %drop_db2_table(tbl_name=&DB2_TMP_TBL3);
		...
		+----------------------------------------------------------------------SASDOC*;
		   ...
		   execute
		   (
		   ALTER TABLE &DB2_TMP_TBL3 ACTIVATE NOT LOGGED INITIALLY
		   )
		   by DB2;
		   execute
		   (
...
*SASDOC--------------------------------------------------------------------------
|JUNE 2012 - TARGET BY DRUG/ DSA AUTOMATION
|EB - CONFIRMED WITH BUSINESS THAT DISCUNTINUED DRUGS DO NOT NEED TO BE EXCLUDED 
+------------------------------------------------------------------------SASDOC*;
      proc sql noprint;
      connect to DB2 as DB2(dsn=&UDBSPRP AUTOCOMMIT=NO);
      execute
      ( 
         INSERT INTO &DB2_TMP_TBL3
            ( DRUG_NDC_ID
             ,NHU_TYPE_CD
             ,DRG_GROUP_SEQ_NB   
             ,DRG_SUB_GRP_SEQ_NB
         ...
         SELECT  DISTINCT
				...
				,0 AS ALL_DRUG_IN
				,1 AS INCLUDE_IN
				,A.&PLATFORM._DRUG_BRAND_CD
				,A.DRUG_GID
         FROM   &DB2_TMP_TBL_DRUG_EDW         A 
               ,&HERCULES..TPHASE_DRG_GRP_DT  B 
         WHERE  A.DRUG_NDC_ID IS NOT NULL
/*           AND  A.NHU_TYPE_CD IS NOT NULL*/
		   AND  B.INITIATIVE_ID = &init_id
...
/*             OR (A.DISCONTINUANCE_DT IS NULL))*/
   ...
   +----------------------------------------------------------------------SASDOC*;
         PROC SQL NOPRINT;
         CONNECT TO DB2 AS DB2(DSN=&UDBSPRP);
         EXECUTE(DELETE FROM &DB2_TMP_TBL3 A
                 WHERE EXISTS
                (SELECT 1
                 FROM &&DB2_TMP_TBL_EDW_&PLATFORM. Z
                 WHERE Z.DRUG_NDC_ID = A.DRUG_NDC_ID                 
/*				 and   Z.NHU_TYPE_CD = A.NHU_TYPE_CD*/
                )) BY DB2;
         %reset_sql_err_cd;
	  ...
	  /* rename output table */
	  %drop_db2_table(tbl_name=&&DB2_TMP_TBL_EDW_&PLATFORM.);
	  %let RTBL=%scan(%str(&&DB2_TMP_TBL_EDW_&PLATFORM.),2,%str(.));

      PROC SQL NOPRINT;
      CONNECT TO DB2 AS DB2(DSN=&UDBSPRP);
      EXECUTE(RENAME TABLE &DB2_TMP_TBL3 TO &RTBL ) BY DB2;
      DISCONNECT FROM DB2;
      QUIT;

	  %mend handle_excluded_drugs;

	  %handle_excluded_drugs(platform=RXCLM);
	  %handle_excluded_drugs(platform=RECAP);
...
%IF (&RX_ADJ=1 OR &RE_ADJ=1 ) 
AND (
%SYSFUNC(EXIST(&DB2_TMP_TBL_EDW_RECAP.)) OR
%SYSFUNC(EXIST(&DB2_TMP_TBL_EDW_RXCLM.)) OR 
((&EXT_DRUG_LIST_IN eq 1) AND %SYSFUNC(EXIST(&DRUG_NDC_TBL.)))
) %THEN %DO;
...
*SASDOC--------------------------------------------------------------------------
|JUNE 2012 - TARGET BY DRUG/ DSA AUTOMATION
|EB - CREATE TEMP DRUG TABLE FOR RECAP AND RXCLAIM 
+------------------------------------------------------------------------SASDOC*;
	%macro create_temp_edw_drug_tbl(platform=);

	%PUT NOTE: Executing create_temp_edw_drug_tbl macro;
    %if &EXT_DRUG_LIST_IN eq 0 %then %do;
		%if (%cmpres(&&DRUG_NDC_TBL_EDW_&PLATFORM.) ne) %then
			%drop_oracle_table(tbl_name=&&DRUG_NDC_TBL_EDW_&PLATFORM.);
   			%PUT NOTE: DROPPING TABLE &&DRUG_NDC_TBL_EDW_&PLATFORM. BECAUSE IT EXISTS;
   	
			PROC SQL NOPRINT;
 			CREATE TABLE &&DRUG_NDC_TBL_EDW_&PLATFORM. AS
 				SELECT DISTINCT A.DRUG_NDC_ID         
                 ,A.DRUG_GID
                 ,A.DRG_GROUP_SEQ_NB   
                 ,A.DRG_SUB_GRP_SEQ_NB
				 ,A.NHU_TYPE_CD 
				 ,A.DRUG_BRAND_CD
 				FROM &&DB2_TMP_TBL_EDW_&PLATFORM. A
				ORDER BY A.DRUG_NDC_ID, A.DRUG_GID;
			QUIT;
	%end;
	%else %do;
			...
			PROC SQL NOPRINT;
 			CREATE TABLE &&DRUG_NDC_TBL_EDW_&PLATFORM. AS
                 ...
                 ,A.DRG_SUB_GRP_SEQ_NB
				 ,A.NHU_TYPE_CD 
				 %IF &PLATFORM. = RECAP %THEN %DO;
				 ,B.RECAP_DRUG_BRAND_CD AS DRUG_BRAND_CD
				 %END;
				 %ELSE %IF &PLATFORM. = RXCLM %THEN %DO;
				 ,B.RXCLM_DRUG_BRAND_CD AS DRUG_BRAND_CD
				 %END;

 			FROM &DRUG_NDC_TBL A
 			LEFT JOIN &DB2_TMP_TBL_DRUG_EDW  B
	...
	%end;
	%mend create_temp_edw_drug_tbl;

	%create_temp_edw_drug_tbl(platform=RECAP);
	%create_temp_edw_drug_tbl(platform=RXCLM);


			%LET pos1a=%INDEX(&DRUG_NDC_TBL_EDW_RECAP,.);
			%LET pos1b=%INDEX(&DRUG_NDC_TBL_EDW_RXCLM,.);
			%LET pos2=%INDEX(&DRUG_NDC_TBL_RX,.);
 			%LET pos3=%INDEX(&DRUG_NDC_TBL_RE,.);
			%LET EDW_table_a = %SUBSTR(&DRUG_NDC_TBL_EDW._RECAP,%EVAL(&pos1a+1));
			%LET EDW_table_b = %SUBSTR(&DRUG_NDC_TBL_EDW._RXCLM,%EVAL(&pos1b+1));
...
+----------------------------------------------------------------------SASDOC*;
		%if &RX_ADJ=1 %then %do;
			...
			execute
			(
			rename &EDW_table_b. to &RX_table.
			)
			by oracle;
			disconnect from oracle;
			quit;
			
			%if NOT %SYSFUNC(EXIST(&DRUG_NDC_TBL_EDW_RECAP.)) AND %SYSFUNC(EXIST(&DRUG_NDC_TBL_RX.)) AND &RE_ADJ=1 %THEN %do;
...
+----------------------------------------------------------------------SASDOC*;
			...
			execute
			(
			rename &EDW_table_a. to &RE_table.
			)
			by oracle;
			disconnect from oracle;
			quit;
			
			%if NOT %SYSFUNC(EXIST(&DRUG_NDC_TBL_EDW_RXCLM.)) AND %SYSFUNC(EXIST(&DRUG_NDC_TBL_RE.)) AND &RX_ADJ=1 %THEN %do;
...
+----------------------------------------------------------------------SASDOC*;
		...
		%IF (&QL_ADJ=1 OR &CC_QL_MIGR_IND.) and %SYSFUNC(EXIST(&DRUG_NDC_TBL.)) %THEN %DO;
   ...
   %IF &RX_ADJ=1 %THEN %DO;
		...
		PROC SQL NOPRINT;
 		CREATE TABLE &DRUG_NDC_TBL_RX AS
 			...
 			FROM &DRUG_NDC_TBL. A
 			LEFT JOIN
				(SELECT DRUG_GID, NDC_CODE AS DRUG_NDC_ID, SUBSTR(GPI_CODE,1,2) AS GPI_GROUP, SUBSTR(GPI_CODE,3,2) AS GPI_CLASS, DRUG_VLD_FLG
	  			FROM &DSS_CLIN..V_DRUG_DENORM) B
					ON  A.DRUG_NDC_ID = input(B.DRUG_NDC_ID,20.)
					AND B.DRUG_VLD_FLG = 'Y'
				    AND A.GPI_GROUP = B.GPI_GROUP
...
%put GPI_NDC_HIS_EDW72 = &GPI_NDC_HIS_EDW72;
	...
	PROC SQL NOPRINT;
	CREATE TABLE &DRUG_NDC_TBL_RE AS
	           SELECT * FROM &NDC_TMP_TBL_RE72 A
			   WHERE NOT EXISTS
                (SELECT 1
...
quit;
	...
	%if &COUNT_MISSING_GIDS > 0 %then %do;
		*SASDOC---------------------------------------------------------------------------------------
		| 06NOV2009 - N.Williams - Produce report of DRUG_GIDS MISSING for this run of the initative. 
		+--------------------------------------------------------------------------------------SASDOC*;
		   ...
		   ods html
		      file =RPTDEL
           ;
		   title1 j=l "DRUG_GIDs Missing on EDW for Initiative# &initiative_id - Report Generated at: &RPT_DTTM";

		   proc print
		      data=&DRUG_NDC_TBL_RE.
		      noobs;
		   where DRUG_GID is missing;
		   var DRUG_GID DRUG_NDC_ID;           
		   run;
		   quit;
		   ods html close;
		   ods listing;
		   ...
		   filename mymail email 
         		to=(&EMAIL_USR)
         		subject="HCE SUPPORT: &PROG_NAME - Notification of Missing Drug Gids for Initiative# &initiative_id" 
				type="text/plain"
...
%IF &DEBUG_FLAG=Y %THEN OPTIONS MLOGIC MPRINT SYMBOLGEN;
