...
/*PROC CONTENTS DATA=DATA_PND.T_&INITIATIVE_ID._&PHASE_SEQ_NB._1_PENDING VARNUM*/
/*  OUT=VARS;*/
/*RUN;*/
/*PROC SORT;BY VARNUM;RUN;*/

/*  PROC FREQ DATA=SAVING.PRINT_DISPOSITION_FILE_&INITIATIVE_ID. ;*/
/*    TABLES DELIVERY_DATE_TIME / MISSING out=disp_check_&INITIATIVE_ID.;*/
/*    TABLES OPPORTUNITY_ID*DELIVERY_DATE_TIME / MISSING out=disp_check_&INITIATIVE_ID.;*/
/*  RUN;*/

  /*PROC SQL;*/
/*  CREATE TABLE DATA_PND.T_&INITIATIVE_ID._&PHASE_SEQ_NB._1_PENDING AS*/
/*  SELECT A.GROUP_CLASS_CD*/
...
/*        ,A.TARGET_DRUG_NM*/
/*        ,A.DRUG_NDC_ID*/
/*        ,A.ALTERNATE_DRUG_NM1*/
/*        ,A.ALTERNATE_DRUG_NM2*/
/*        ,A.ALTERNATE_DRUG_NM3*/
/*        ,A.ALTERNATE_DRUG_NM4*/
/*        ,A.ALTERNATE_DRUG_NM5*/
/*        ,A.SRC_INSRD_MBR_ID*/
/*        ,A.EXPIRATION_DATE*/
/*        ,A.PHY_TOLL_FREE_NUM*/
/*        ,A.GENDER*/
...
/*        ,A.CLIENT_LEVEL_8*/
/*        ,A.CLIENT_LEVEL_9*/
/*        ,A.ALTERNATE_DRUG_NDC1*/
/*        ,A.ALTERNATE_DRUG_NDC2*/
/*        ,A.ALTERNATE_DRUG_NDC3*/
/*        ,A.ALTERNATE_DRUG_NDC4*/
/*        ,A.ALTERNATE_DRUG_NDC5*/
/*        ,A.GROUP_CD*/
/*        ,A.SUBJECT_ID*/
/*        ,A.DTE_OF_BRTH*/
/*        ,A.seed_row*/
...
/*        ,A.COUNTRY_CD*/
/*        ,A.data_quality_cd*/
/*  FROM DATA_PND.T_&INITIATIVE_ID._&PHASE_SEQ_NB._1_PENDING A,*/
/*       DATA_PND.T_&INITIATIVE_ID._&PHASE_SEQ_NB._1_RESULTS B*/
...
%*SASDOC -----------------------------------------------------------------------
 ...
 +----------------------------------------------------------------------SASDOC*;
	...
	%*SASDOC -----------------------------------------------------------------------
	 ...
	 +----------------------------------------------------------------------SASDOC*;
	 PROC SQL;
		 CREATE TABLE SAVING.PRINT_DISPOSITION_FILE_&INITIATIVE_ID 
		 (SOURCE CHAR(5),
     OPPORTUNITY_ID NUM ,
...
%*SASDOC -----------------------------------------------------------------------
 ...
 | 
 +----------------------------------------------------------------------SASDOC*;
%LET DSID=%SYSFUNC(OPEN(DATA_PND.T_&INITIATIVE_ID._&PHASE_SEQ_NB._1)); 
...
%IF &&DATA_QUALITY_CD&Q=1 OR &&DATA_QUALITY_CD&Q=2 %THEN %DO ;
	%IF &PROGRAM_ID=5252 or &PROGRAM_ID=5253 or &PROGRAM_ID=5254 or &PROGRAM_ID=5255 or &PROGRAM_ID=5256 or 
      ...
      &PROGRAM_ID=5357 or &PROGRAM_ID=5368 
   %then %do;
     %LET CEE_OPP_STATUS_CD=1 ;
     %LET CEE_OPP_STATUS_RSN_CD = 0;
  %end;
	%IF &PROGRAM_ID=5371 
   %then %do;
     %LET CEE_OPP_STATUS_CD=1 ;
...
PROC SQL ;
  INSERT INTO SAVING.PRINT_DISPOSITION_FILE_&initiative_id. 
...
SET SAVING.PRINT_DISPOSITION_FILE_&initiative_id.  ;
SOURCE1 = left(SOURCE);
