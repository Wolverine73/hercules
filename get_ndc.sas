...
|								   	
|           06NOV2009 - N.Williams - Hercules Version 2.2.03
|									 Added logic produce a report of missing drug gids for drugs  
|                                    when running drug setup for EDW initiative.
+----------------------------------------------------------------------HEADER*;

*SASDOC-----------------------------------------------------------------------
| Macro is modifed to handle RX and RE claims too along with QL claims.
| S.Yaramada 31MAR2008
+----------------------------------------------------------------------SASDOC*;
%macro get_ndc(DRUG_NDC_TBL=,
			 DRUG_NDC_TBL_RX=,
			 DRUG_NDC_TBL_RE=, 			
             CLAIM_DATE_TBL=,	   			
   ...
   %do;
      ...
      FROM
         ...
         SELECT  A.DRG_GROUP_SEQ_NB
                ...
                ,CAST(NULLIF(SUBSTR(B.DRUG_ID,01,2),'  ') AS CHAR(2)) AS GPI_GROUP
                ,CAST(NULLIF(SUBSTR(B.DRUG_ID,03,2),'  ') AS CHAR(2)) AS GPI_CLASS
                ,CAST(NULLIF(SUBSTR(B.DRUG_ID,05,2),'  ') AS CHAR(2)) AS GPI_SUBCLASS
                ,CAST(NULLIF(SUBSTR(B.DRUG_ID,07,2),'  ') AS CHAR(2)) AS GPI_NAME
                ,CAST(NULLIF(SUBSTR(B.DRUG_ID,09,2),'  ') AS CHAR(2)) AS GPI_NAME_EXTENSION
                ,CAST(NULLIF(SUBSTR(B.DRUG_ID,11,2),'  ') AS CHAR(2)) AS GPI_FORM
                ,CAST(NULLIF(SUBSTR(B.DRUG_ID,13,2),'  ') AS CHAR(2)) AS GPI_STRENGTH
                ,B.NHU_TYPE_CD
...
%put NOTE: GPI_NDC_HIS = &GPI_NDC_HIS; 
   ...
   (
	  ...
	  SELECT DISTINCT B.DRUG_NDC_ID
                     ,B.NHU_TYPE_CD
                     ,CAST(B.GPI_GROUP AS CHAR(2))
                     ,CAST(B.GPI_CLASS AS CHAR(2))
            FROM 
				(
				 SELECT DISTINCT GPI_CD
	   				  			,CAST(NULLIF(SUBSTR(GPI_CD,01,2),'  ') AS CHAR(2)) AS GPI_GROUP
...
%put NDC_TMP_TBL = &NDC_TMP_TBL.;
   ...
   execute
      ...
      SELECT  DISTINCT
                    DRUG_NDC_ID
                   ,NHU_TYPE_CD
                   ,CAST(GPI_GROUP AS CHAR(2)) AS GPI_GROUP
                   ,CAST(GPI_CLASS AS CHAR(2)) AS GPI_CLASS
            ...
            WHERE   (    TIMESTAMPDIFF(256,CHAR(CURRENT TIMESTAMP - TIMESTAMP_ISO(DISCONTINUANCE_DT))) < 3
              ...
              AND   (DRUG_MAINT_IN = 1
			   OR	DGH_EXT_MNT_IN = 1)
            GROUP BY DRUG_NDC_ID, NHU_TYPE_CD, GPI_GROUP
                   ,GPI_CLASS
            ORDER BY DRUG_NDC_ID, NHU_TYPE_CD
      )
   by DB2;
 		...
 		CREATE TABLE &DRUG_NDC_TBL_RX AS
 			...
 			LEFT JOIN
				(SELECT DRUG_GID,
						...
						put(SUBSTR(GPI_CODE,1,2),$2.) AS GPI_GROUP , 
						put(SUBSTR(GPI_CODE,3,2),$2.) AS GPI_CLASS, 
						DRUG_VLD_FLG
	  			FROM &DSS_CLIN..V_DRUG_DENORM) B
					ON  A.DRUG_NDC_ID = input(B.DRUG_NDC_ID,20.)
					AND B.DRUG_VLD_FLG = 'Y'
...
%put GPI_NDC_HIS_EDW72 = &GPI_NDC_HIS_EDW72;
	...
	SELECT DISTINCT   B.NDC_CODE AS DRUG_NDC_ID
            ...
            FROM 
				(
				 SELECT DISTINCT GPI_CD
                ,put(NULLIF(SUBSTR(B.DRUG_ID,01,2),'  '), $2.  AS GPI_GROUP
                ,put(NULLIF(SUBSTR(B.DRUG_ID,03,2),'  '), $2.  AS GPI_CLASS
                ,put(NULLIF(SUBSTR(B.DRUG_ID,05,2),'  '), $2.  AS GPI_SUBCLASS
                ,put(NULLIF(SUBSTR(B.DRUG_ID,07,2),'  '), $2.  AS GPI_NAME
                ,put(NULLIF(SUBSTR(B.DRUG_ID,09,2),'  '), $2.  AS GPI_NAME_EXTENSION
                ,put(NULLIF(SUBSTR(B.DRUG_ID,11,2),'  '), $2.  AS GPI_FORM
                ,put(NULLIF(SUBSTR(B.DRUG_ID,13,2),'  '), $2.  AS GPI_STRENGTH
								,DRUG_CATEGORY_ID
	...
	%if &COUNT_MISSING_GIDS > 0 %then %do;
		*SASDOC---------------------------------------------------------------------------------------
		...
		+--------------------------------------------------------------------------------------SASDOC*;
		   ...
		   ods listing close;
		   ods html file=RPTDEL;

		   proc print noobs data=&DRUG_NDC_TBL_RE.;
