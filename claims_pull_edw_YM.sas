...
|
+------------------------------------------------------------------------SASDOC*;
%MACRO CLAIMS_PULL_EDW_YM(DRUG_NDC_TABLE_RX = &ORA_TMP..&TABLE_PREFIX._NDC_RX,
...
%LET NEW_ADD_BASE_COLUMNS=%STR(MBR.MBR_ID, 
...
%IF &PROGRAM_ID NE 105 %THEN %DO;
		...
		%DROP_ORACLE_TABLE(TBL_NAME = &ORA_TMP..TMP_CLAIMS_PULL2_&INITIATIVE_ID._&ADJ_ENGINE.);
		PROC SQL;
			CONNECT TO ORACLE(PATH=&GOLD PRESERVE_COMMENTS);
			EXECUTE
			(
				CREATE TABLE &ORA_TMP..TMP_CLAIMS_PULL2_&INITIATIVE_ID._&ADJ_ENGINE. AS
				SELECT 	/* + ORDERED */
						...
						,DRUG.DRUG_GID
/*						,DRUG.GCN_CODE */ /*YM:As per design doc uncommneted and used  GCN_NBR AS GCN_CODE  below*/ 
						,DRUG.GCN_NBR 
...
/*						,DRUG.NDC_CODE AS DRUG_NDC_ID*/
						...
						/*            ,&REFILL_QTY.*/
            			,0 AS REFILL_FILL_QY /* CAST(CLAIM.NEW_REFIL_CODE AS INT) AS REFILL_FILL_QY */
...
/*YM: NEW ADDITIONAL FIELDS AS PER ADD BASE COLUMNS */
				...
				FROM 	
						...
						,&DSS_CLIN..V_MBR MBR /*YM:ADD BASE NEW TABLE ADDED  */
				WHERE	CLAIM.DSPND_DATE BETWEEN &CLM_BEGIN_DT_CONV AND &CLM_END_DT_CONV 
					...
					AND CLAIM.MBR_GID = MBR.MBR_GID /*YM:ADD BASE NEW CONDITION ADDED  */
					&DS_STRING_RX_RE.
	...
	%DROP_ORACLE_TABLE(TBL_NAME = &ORA_TMP..TMP_CLAIMS_PULL1_&INITIATIVE_ID._&ADJ_ENGINE.); 
		PROC SQL;
			CONNECT TO ORACLE(PATH=&GOLD PRESERVE_COMMENTS);
			EXECUTE
			( CREATE TABLE &ORA_TMP..TMP_CLAIMS_PULL1_&INITIATIVE_ID._&ADJ_ENGINE. AS
				     SELECT CLAIMS.* ,FRMLY.FRMLY_NB AS FORMULARY_TX
					 FROM &ORA_TMP..TMP_CLAIMS_PULL2_&INITIATIVE_ID._&ADJ_ENGINE. CLAIMS
					       LEFT JOIN
				           &DSS_CLIN..V_FRMLY_HDR FRMLY 
       ...
       PROC SQL;
			...
			( CREATE TABLE &ORA_TMP..TMP_CLAIMS_PULL_&INITIATIVE_ID._&ADJ_ENGINE. AS
					 SELECT CLAIM.*,
						   ...
						   ELSE ' '
					       END AS DEA_NB
					FROM &ORA_TMP..TMP_CLAIMS_PULL1_&INITIATIVE_ID._&ADJ_ENGINE. CLAIM
...
/*							,MBR.MBR_ID*/ /* YM: commented becoz its already added in claims table */
							,MBR.MBR_FIRST_NM
							,MBR.MBR_LAST_NM				
							,MBR.ADDR_LINE1_TXT			
							,MBR.ADDR_LINE2_TXT			
...
DELIVERY_SYSTEM_CD, PT_BENEFICIARY_ID, CDH_BENEFICIARY_ID, MBR_ID, MBR_FIRST_NM, 
MBR_LAST_NM, ADDR_LINE1_TXT, ADDR_LINE2_TXT, ADDR_CITY_NM, ADDR_ST_CD, ADDR_ZIP_CD, 
SRC_SUFFX_PRSN_CD,&NEW_ADD_BASE_COL1.) 
SELECT 
ADJ_ENGINE, ALGN_LVL_GID_KEY, CLIENT_ID, CLIENT_NM, CLIENT_LEVEL_1, CLIENT_LEVEL_2, 
CLIENT_LEVEL_3, PRACTITIONER_ID, PRESCRIBER_ID, ENTITY_IND, DEGR_1_CD, NABP_CODE_6, 
PAYER_ID, MBR_GID, PHMCY_GID, PRCTR_GID, DSPND_DATE, BATCH_DATE, MEMBER_COST_AT, 
