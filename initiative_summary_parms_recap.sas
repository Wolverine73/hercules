/*HEADER------------------------------------------------------------------------
|
| PROGRAM:  initiative_summary_parms_recap.sas
|
...
PROC PRINTTO LOG="/PRG/sas&sysmode.1/hercules/reports/initiative_summary_parms_recap.log" NEW;
RUN;
QUIT; **/
...
define F_INCLUDE_IN                / order
   ...
   style=[cellwidth=0.52in
          font_weight=medium
          just=l];
define INSURANCE_CD             / order
   "^S={font_weight=bold
        background =&_hdr_bg
        just       =c}Insurance*Cd^S={}"
   style=[cellwidth=0.80in
          font_weight=medium
          just=r];
define CARRIER_ID          		/ order
   "^S={font_weight=bold
        background =&_hdr_bg
        just       =c}Carrier*Id^S={}"
   style=[cellwidth=0.80in
