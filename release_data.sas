...
|
|       
|           19SEP2007 - Carl J Starks - Hercules Version  1.5.02
...
|
|       
|           10DEC2007 - Carl J Starks - Hercules Version  1.5.03
...
|
|       
|           18APR2008 - Carl J Starks - Hercules Version  2.0.01
...
|        
|           22JUL2008 - Gregory Dudley
|                       Replaced %sysfunc(compress( with %cmpres(
|                
|           01OCT2008 - Sudha Yaramada - Hercules Version  2.1.2.00
...
|           
|           18NOV2008 - Suresh Ramanathan - Hercules Version  2.1.2.01
|                       fix added to release initiatives that were created/executed
...
|           
|           26FEB2009 - Brian Stropich - Hercules Version  2.5.03
|                       Added an additional condition to set ds_pnd_where to 1, 2, and 4 to allow
|                       contaminated EDW members for analysis but not for final release
|
|           19NOV2009 - G.Dudley  - Added logic to add the "coe_" prefix to the 
|                                   output file for GCI and TGAM program IDs  
|
|           09SEP2010 - D. Palmer - added PA program id 5371 to EOMS program list.
|
|			13JAN2011 - Sathishkumar Veeraswamy (SKV) - iBenefit 3.0 - Added an additional condition to 
|																set ds_pnd_where when File id = 35.
|
|           19JUN2011 - Brian Stropich - Added logic to sync ibenefit main and detail with
|                       the same recipient IDs.  Search for '20110619 - Change' in the program
|                       for the changes and details.
|
|           08JUL2011 - Sathishkumar Veeraswamy (SKV) - PSG July 2011 Release
|						Added logic to not to include new PSG mailing file columns 
|						if created date of pending initiatives is earlier than the deployment date.
|
|			16AUG2013 - Arjun Kolakotla/JA - BG Brand Gap Changes to negative_formulary requires new
|						columns to be added to pending and release files. New table: hercules.therc_release_field
|						was created to make it easy to add any columns to any pending/release files.
+-----------------------------------------------------------------------HEADER*/
...
%macro release_data(init_id=&initiative_id,
  ...
  %*====================================================================SASDOC*;
    ...
    proc sql noprint;
      ...
      where  initiative_id = &init_id    and
		...
		%PUT NOTE: _destination_cd=&_destination_cd.;
		%PUT NOTE: file_id=&file_id.;
		%PUT NOTE: file_usage_cd=&file_usage_cd.;
		%PUT NOTE: hsu_usr_id=&hsu_usr_id.;
		%PUT NOTE: hsc_usr_id=&hsc_usr_id.;
		%PUT NOTE: release_status_cd=&release_status_cd.;
		%PUT NOTE: n_rows=&n_rows.;
...
/*	AK/JA - BG 16AUG2013 - Program specific file layout - additional fields*/

proc sql noprint;
	select max(field_new_seq_nb) into: max_new_field_seq_nb
	from work.required_fields
where field_new_seq_nb < 999;
quit;

%put NOTE: MAX_NEW_FIELD_SEQ_NB - &max_new_field_seq_nb;

	 proc sql noprint;
      create table work.additional_release_fields  as
      select  distinct rcvr.initiative_id,
               rcvr.phase_seq_nb,
               rcvr.cmctn_role_cd,
               rcvr.letters_sent_qy,
               rcvr.file_usage_cd,
               ...
               rcvr.hsc_usr_id,
               rcvr.hsu_usr_id,
			   'ADDED FIELD'          					 as file_short_tx informat=$30.,
               'ADDED FIELD TO PROGRAM SPECIFIC LAYOUT'  as file_long_tx informat=$255.,
               req.seq_nb			          		  as field_seq_nb,      
               fdes.format_sas_tx,
               upcase(fdes.field_nm)  as field_nm,
               fdes.short_tx          as field_short_tx,
               fdes.long_tx           as field_long_tx
      ...
      from     hercules.tphase_rvr_file AS rcvr,	
			   hercules.therc_release_field as req,
               hercules.tfield_description  AS	fdes

      where    rcvr.initiative_id   = &init_id.    and
               rcvr.phase_seq_nb    = &phase_id.     and
               rcvr.cmctn_role_cd   = &com_cd.    and
			   req.field_id			= fdes.field_id		 and
			   req.file_id 			= rcvr.file_id		 
;quit;


data work.additional_release_fields;
set work.additional_release_fields;
retain field_new_seq_nb &max_new_field_seq_nb.;
field_new_seq_nb+1;
run;


proc append base = work.required_fields data = work.additional_release_fields;
run;

proc sort data = work.required_fields;
by field_new_seq_nb;
run;

/*	AK/JA - BG - Program specific file layout - additional fields*/


/*SASDOC - AK - added 03MAR2013 - Brand generic column became part of the standard file layout 99 in the additional columns
								  project and the initiatives that were generated before 13FEB2013 and the initiatives that 
								  were out of scope for the aforementioned project do not have brand_generic column on the pending
								  dataset. Hence this column is removed from the required fields dataset for such cases */

/*	Get the date of the initiative's completion	*/
proc sql;
select datepart(job_complete_ts) into: job_complete_ts
from hercules.tinitiative_phase where
initiative_id = &initiative_id.;
quit;


proc sql;
select program_id into: prgm_id from hercules.tinitiative 
where initiative_id = &initiative_id.;
quit;

/*	Need to exclude brand_generic look up for pharmacy advisor, abpd. If any other program needs this treatment,
	add it to the list below	*/
data _null_;
if &prgm_id. in (5371 5368) then do;
call symput('brand_generic_flag',1);
end;
else do;
call symput('brand_generic_flag',0);
end;
run;

%PUT job_complete_ts = &job_complete_ts;
%PUT prgm_id = &prgm_id;
%PUT brand_generic_flag = &brand_generic_flag;


%if &job_complete_ts. < 19402  or &brand_generic_flag.=1   /* 19402 = 13FEB2013*/
%then %do;
%put Brand_Generic is not part of pending dataset, so will not be in the required fields dataset;


/*	This data step will exclude brand_generic from the required fields dataset	*/
data work.required_fields;
set work.required_fields;
where upcase(field_nm) ne 'BRAND_GENERIC';
run;

%end;

%else %do;
%put Brand_Generic is part of layout, so will be in the required fields dataset;
%end;

/*	End AK added - 03MAR2013	*/



/*	AK added - to ignore the PRVCY_ADDR_USE_IND for older initiatives - HMSA project	- NOV2013	*/
/*%Put ak = &file_seq_nb;*/
%if ((&prgm_id ne 5368 or &prgm_id ne 5371) and &job_complete_ts. <=  19697) or (&cmctn_role_cd ne 1) or (&file_seq_nb > 1) or (&file_id =  26) %then %do;
/*19697 - 05DEC2013*/
/*To check date, plug in date below and check the macro variable x_date_chk value  */
/*data _null_;*/
/*call symput ('x_date_chk','05dec2013'd);*/
/*;run;*/
/*%put &x_date_chk;*/

data work.required_fields;
set work.required_fields;
where upcase(field_nm) ne 'PRVCY_ADDR_USE_IND';
run;

%put This initiative was created before the HMSA project, hence is not expected to have the PRVCY_ADDR_USE_IND column in the pending dataset. It is not
	required on the standard file layout.
OR  This is not a participant dataset, so it will not have the privacyindicator	
OR  This is an ibenefit2.0 initiative	;

%end;

/*	End AK added - NOV2013	*/
