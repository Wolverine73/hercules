...
| 04nov2010 - Created custom version for GSTP that is using mbr_id
+----------------------------------------------------------------------HEADER*;
%macro get_data_quality_gstp(TBL_NAME_IN=, TBL_NAME_OUT=);
...
+-----------------------------------------------------------------------SASDOC*;
        ...
        from    (
                select distinct  RECIPIENT_ID, MBR_ID, CLIENT_LEVEL_1, ZIP_CD, RVR_FIRST_NM, RVR_LAST_NM
...
+-----------------------------------------------------------------------SASDOC*;
        ...
        group by RECIPIENT_ID, MBR_ID, CLIENT_LEVEL_1) as B
on      (A.RECIPIENT_ID eq B.RECIPIENT_ID and A.MBR_ID eq B.MBR_ID AND A.CLIENT_LEVEL_1 EQ B.CLIENT_LEVEL_1);
quit;
