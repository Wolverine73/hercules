...
+------------------------------------------------------------------------SASDOC*;
%MACRO CLAIMS_PULL_EDW_24MAR2009(DRUG_NDC_TABLE_RX = &ORA_TMP..&TABLE_PREFIX._NDC_RX,
					   DRUG_NDC_TABLE_RE = &ORA_TMP..&TABLE_PREFIX._NDC_RE, 
					   DRUG_RVW_DATES_TABLE = &ORA_TMP..&TABLE_PREFIX._RVW_DATES,
					   RESOLVE_CLIENT_TABLE_RX = &ORA_TMP..&TABLE_PREFIX._RX ,
					   RESOLVE_CLIENT_TABLE_RE = &ORA_TMP..&TABLE_PREFIX._RE
...
+------------------------------------------------------------------------SASDOC*;
        ...
        SELECT %BQUOTE(/)%BQUOTE(*)+ ORDERED full(claim) use_hash(vclm) full(vclm)  use_hash(clt) full(clt) 
                use_hash(drug) full(drug) use_hash(prctr) full(prctr) use_hash(phmcy) full(phmcy) %BQUOTE(*)%BQUOTE(/)
           ...
           WHERE	CLAIM.DSPND_DATE BETWEEN &CLM_BEGIN_DT_CONV AND &CLM_END_DT_CONV 
             ...
             AND vclm.algn_lvl_gid = clt.algn_lvl_gid_key
             AND clt.payer_id = vclm.payer_id
...
%MEND CLAIMS_PULL_EDW_24MAR2009;
