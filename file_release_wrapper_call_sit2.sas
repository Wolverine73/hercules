...
/*  %update_request_ts(start);*/
    ...
    %*==================================================================SASDOC*;
	      ...
	      **custom code to release all data_quality_cd values change 3s to 2s.;

        PROC SQL;
          UPDATE DATA_PND.T_&initiative_id._1_1
            SET  DATA_QUALITY_CD = 2
            WHERE DATA_QUALITY_CD = 3
          ;
          quit;

    %release_data_sit2(init_id=&init_id, phase_id=&phase_id, com_cd=&com_cd);
...
options sysparm='INITIATIVE_ID=8360 PHASE_SEQ_NB=1 REQUEST_ID=8358 REPORT_ID=999 DOCUMENT_LOC_CD=1 CMCTN_ROLE_CD=1 HSC_USR_ID=QCPI208';
