...
%MACRO pull_claims_for_ql_dsa(tbl_name_in1=, tbl_name_in2=,tbl_name_in3=,
...
%END;
%ELSE %DO;
/*	%LET WHERECONS = %STR(AND A.CLIENT_ID =11458);*/
/*	%IF &CC_QL_MIGR_IND. AND &TASK_ID. =11 %THEN %DO;*/
/*		%IF %SYSFUNC(EXIST(&DB2_TMP..&TABLE_PREFIX._QL_MIGR))*/
/*			%THEN %DO;*/
/*				PROC SQL;*/
/*					SELECT DISTINCT CLT_LEVEL_1 INTO :QL_CLT_ID_MIGR*/
/*					FROM &ORA_TMP..&TABLE_PREFIX._QL_MIGR;*/
/*				QUIT;*/
/*				%LET WHERECONS = %STR( AND A.CLIENT_ID NOT IN (&QL_CLT_ID_MIGR.) );*/
/*		%END;*/
/*		%ELSE %DO;*/
/*				%LET WHERECONS = %STR();*/
/*		%END;*/
...
EXECUTE(INSERT INTO &tbl_name_out1._A
        WITH PTS AS
                (SELECT          A.NTW_PRESCRIBER_ID AS PRESCRIBER_ID,
								 ...
								 RTRIM(A.NTW_PRESCRIBER_ID)||JULIAN_DAY(MAX(A.FILL_DT))||MAX(A.REFILL_NB) AS RXKEY
...
/*Q2X added ADJ_ENGINE_CD since QL migrated clients change code to 2 after migration*/
							...
							AND E.RXKEY = RTRIM(X.NTW_PRESCRIBER_ID)||RTRIM(JULIAN_DAY(X.FILL_DT))||RTRIM(X.REFILL_NB)
                            AND X.RX_COUNT_QY > 0
