/**HEADER -------------------------------------------------------------------------------------------------
  ...
  |           FOR DSA/NCQA
  |
  |           November 2013 - Ray Pileckis - code chg to delete duplicates 
  |
  +-------------------------------------------------------------------------------------------------HEADER*/

%MACRO CHECK_DOCUMENT;
...
*SASDOC -----------------------------------------------------------------------------------
  ...
  +--------------------------------------------------------------------------------SASDOC*;
	...
	| YM:Nov29,2012: Changed &MODMAIL.NE  condition to &MODMAIL.EQ  (Equal to zero)
	|to fix the bug as client_level_1 is added into claims as per Add base columns project 
 	+------------------------------------------------------------------------------SASDOC;
...
%*SASDOC -----------------------------------------------------------------------
 ...
 +-----------------------------------------------------------------------SASDOC*;
    ...
    /* 11.2103 remove duplicates - begin */

	%IF (&PROGRAM_ID EQ 105 ) %THEN %DO;
		%LET ADJ_CONS = %STR(AND A.ADJ_ENGINE = "&ADJ_ENGINE.");
	%END;

    /* 11.2103 remove duplicates - end */
...
*SASDOC -----------------------------------------------------------------------------------
  | DROP THE TABLES THAT WERE RENAMED DURING UPDATES IF NO ERRORS WERE ENCOUNTERED.
...
proc datasets nolist;
  ...
  out=WORK.PND_TABLES(
    keep=MEMNAME
    where=(MEMNAME like "CHKDOC_RN_&SYSJOBID._%")
  );
  contents
  noprint
  data=DATA_RES._ALL_
...
proc sort data=WORK.PND_TABLES nodup;
  by MEMNAME;
run;

proc sort data=WORK.RES_TABLES nodup;
  by MEMNAME;
run;

data _NULL_;
  set WORK.PND_TABLES end=DONE;
  call symputx(cats('PND',_N_),MEMNAME);
  if(DONE) then call symputx('PND_TABLES',_N_);
run;

data _NULL_;
  set WORK.RES_TABLES end=DONE;
  call symputx(cats('RES',_N_),MEMNAME);
  if(DONE) then call symputx('RES_TABLES',_N_);
run;

proc sql;
  %do N=1 %to &PND_TABLES;
    drop table DATA_PND.&&PND&N;
  %end;
  %do N=1 %to &RES_TABLES;
    drop table DATA_RES.&&RES&N;
  %end;
quit;
