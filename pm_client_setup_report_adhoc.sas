...
OPTIONS SYSPARM='REQUEST_ID=108113 RPT_PROGRAM_ID=5246 RPT_TASK_ID=65 HSC_USR_ID=QCPUD92G';
...
PROC SQL NOPRINT;
	...
	WHERE PROGRAM_ID = &RPT_PROGRAM_ID. AND TASK_ID = &RPT_TASK_ID.;
QUIT;
...
PROC SQL NOPRINT;
	...
	AND C.PROGRAM_ID= &RPT_PROGRAM_ID.
...
%macro ql_clients;
	...
	%end;
	%else %do;
		%put note: no rows for QL found;
		%email_parms( EM_TO=&_em_to_user
		      ,EM_CC="Hercules.Support@caremark.com"
      		,EM_SUBJECT="QL Client Setup Report for &RPT_PROGRAM_ID. and &RPT_TASK_ID. - no QL clients setup for this task"
      		,EM_MSG="There are no QL clients setup for &RPT_PROGRAM_ID.-&RPT_PROGRAM_NAME. and &RPT_TASK_ID.-&RPT_TASK_NAME. ."
			,EM_ATTACH="/herc&sysmode/data/hercules/reports/ql_client_setup_&RPT_PROGRAM_ID._&RPT_TASK_ID..pdf"  
			ct="application/pdf");
...
/*       AND A.EFFECTIVE_DT <= &CURRENT_DT.*/
	...
	%end;
	%else %do;
		%put note: no rows for RX found;
		%email_parms( EM_TO=&_em_to_user
	      	,EM_CC="Hercules.Support@caremark.com"
      		,EM_SUBJECT="RX Client Setup Report for &RPT_PROGRAM_ID. and &RPT_TASK_ID. - no RX clients setup for this task"
      		,EM_MSG="There are no RX clients setup for &RPT_PROGRAM_ID.-&RPT_PROGRAM_NAME. and &RPT_TASK_ID.-&RPT_TASK_NAME. ."
			,EM_ATTACH="/herc&sysmode/data/hercules/reports/rx_client_setup_&RPT_PROGRAM_ID._&RPT_TASK_ID..pdf"  
			ct="application/pdf");
...
%macro re_clients;
	...
	%end;
	%else %do;
			%put note: no rows for RE found;
			%email_parms( EM_TO=&_em_to_user
      		,EM_CC="Hercules.Support@caremark.com"
      		,EM_SUBJECT="RE Client Setup Report for &RPT_PROGRAM_ID. and &RPT_TASK_ID. - no RE clients setup for this task"
      		,EM_MSG="There are no RE clients setup for &RPT_PROGRAM_ID.-&RPT_PROGRAM_NAME. and &RPT_TASK_ID.-&RPT_TASK_NAME. ."
			,EM_ATTACH="/herc&sysmode/data/hercules/reports/re_client_setup_&RPT_PROGRAM_ID._&RPT_TASK_ID..pdf"  
			ct="application/pdf");
