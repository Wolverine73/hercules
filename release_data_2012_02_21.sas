...
options SYMBOLGEN MLOGIC MPRINT MLOGICNEST MPRINTNEST SOURCE2;
	...
	/** 02DEC2008: MODIFIED THE ABOVE FIX TO MAKE IT WORK FOR SAS VERSION 9 **/
       ...
       *================================================================SASDOC*/
		DATA _NULL_;
			...
			ELSE DO;
				CALL SYMPUT('IB3_CONS_HISTORY','AND SEQ_NB NOT IN (134,135,136,137,138,139,140,141,142,143,
...
/*			   %reset_sql_err_cd;*/
	  ...
	  %let err_fl=1;
          ...
          %if &file_id. = 26 and &FILE_SEQ_NB=1 %then %do;  **20110619 Change - ibenefit 2.0 - main file;
            proc sql noprint;
	     ...
	     proc surveyselect data=work.final method=srs
          ...
          %if &file_id. = 26 and &FILE_SEQ_NB=2 %then %do;  **20110619 Change - ibenefit 2.0 - detail file;
...
/*		   		%reset_sql_err_cd;*/
	    ...
	    %put NOTE:(&sysmacroname): Task output will be directory to %upcase(&ftp_host):&destination_sub_dir..;
	      ...
	      %if &_destination_cd^=2 %then %do;
            ...
            %then %do;
                   %let file_name_main=&adhoc_root.1/hercules/coe_t_&init_id._&phase_id._&com_cd._&file_id;
		     	  %end;
		     	  %else %do;
