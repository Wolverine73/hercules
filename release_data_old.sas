...
options symbolgen;
%macro release_data_old(init_id=&initiative_id,
...
options symbolgen mlogic mprint;
  ...
  %*====================================================================SASDOC*;
    ...
    proc sql noprint;
	  ...
	  %* SR - Version 2.1 - 8/22/2008 - commented out the count(*) in the query as
	  %*                                it requires remerging summary statistics
	  %*                                Used &sqlobs automatic MV instead to get 
	  %*                                row counts
      %*================================================================SASDOC* ;
	 ...
	 proc sql noprint;
      ...
      from     &hercules..tphase_rvr_file 					AS rcvr,	
              ...
              (select &file_id99 as file_id,
               ...
               from   &hercules..tfile_base_field(WHERE=(seq_nb=&FILE_SEQ_NB. * seq_nb))  AS base
			   union corresponding
