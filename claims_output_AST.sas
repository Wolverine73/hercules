...
+-----------------------------------------------------------------------------------------------------------HEADER*/
%MACRO CLAIMS_OUTPUT_AST(TBL_NM_OUT=,INPT_TBL=,INPT_TBL2=,PLATFORM=,SRC_CD=,
...
SELECT * FROM CONNECTION TO &PLATFORM
           (SELECT DISTINCT
			%IF &SRC_CD=83 %THEN %DO;
											 ...
											 A.GROUP_CD as CLIENT_LEVEL_2,/* NEW FIELDS FROM CLAIM_HIS_TBL */
											 A.BLG_REPORTING_CD as CLIENT_LEVEL_3,
											 ...
											 A.FORMULARY_ID as FORMULARY_TX,
