...
+-----------------------------------------------------------------------HEADER*/
%MACRO CREATE_BASE_FILE_NO_VBENEF(TBL_NAME_IN = ,
                        TBL_NAME_IN_LIST = );
...
%IF (&CMCTN_ROLE_CD. NE 2) OR (&PROGRAM_ID EQ 105 
...
%include "/herc%lowcase(&SYSMODE)/prg/hercules/macros/add_rocc_cd.sas";
...
%IF &FILE_SEQ_NB=1 %THEN %DO;
    ...
    OR (&PROGRAM_ID EQ 106 AND &TASK_ID EQ 21))) %THEN %DO;
	   		%include "/herc%lowcase(&sysmode)/prg/hercules/macros/additional_fields.sas";
...
%MEND CREATE_BASE_FILE_NO_VBENEF;
