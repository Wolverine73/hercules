...
OPTIONS SYSPARM='INITIATIVE_ID=11156 PHASE_SEQ_NB=1';
...
PROC SQL;
  ...
  ,CLAIM.UNIT_QTY                      AS QUANTITY
  ,CLAIM.DAYS_SPLY                     AS DAYS_SUPPLY
  ,FLOOR(MONTHS_BETWEEN(CLAIM.DSPND_DATE,CLAIM.PTNT_BRTH_DT)/12) AS AGE
  ,CLAIM.RX_NBR                        AS RX_NBR
...
AND ( (CLNT.SYS_CD ='R' AND ALGN.RPT_OPT1_CD = CLNT.RPT_OPT1_CD) OR
   ...
   AND CLAIM.DSPND_DATE BETWEEN TO_DATE(&BDATE,'YYYY-MM-DD')
      AND TO_DATE(&EDATE,'YYYY-MM-DD')
   AND (CLAIM.MBR_SUFFX_FLG = 'Y'
      OR CLAIM.MBR_SUFFX_FLG IS NULL)
...
PROC SORT DATA=CLAIMS;
  BY PHMCY_GID RX_NBR DISP_DT DRUG_KEY NDC
...
DATA CLAIMS_3;
FORMAT M_DOB YYMMDD10.;
LENGTH CLNT_LVL1 CLNT_LVL2 CLNT_LVL3 $21.;
...
IF SYS_CD='R' THEN DO;
   ...
   CLNT_LVL1=OPT1;
   CLNT_LVL2=LVL2;
   CLNT_LVL3=LEFT(PUT(QL_CPG_ID,20.));
...
, A.MDDB_MS_SS_CD
, A.DRUG_KEY
, A.QL_DRUG_ABBR_PROD_NM
, A.QL_DRUG_ABBR_DSG_NM
, A.QL_DRUG_ABBR_STRG_NM
, A.DELIVERY_CD
, A.PHYS_MAIL_FLAG
, A.PHYS_MAIL_FLAG_2
, A.MBR_MAIL_FLAG
...
IF A AND NOT B;
IF SYS_CD='Q' THEN DO;
   CLNT_LVL1=LEFT(PUT(QL_CPG_ID,20.));
   CLNT_LVL2='';
   CLNT_LVL3='';   
END;
