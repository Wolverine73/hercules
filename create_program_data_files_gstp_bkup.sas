...
%MACRO CREATE_PROGRAM_DATA_FILES_GSTP;
	...
	%LET GSTP_DALCDCP_LOC = /DATA/sas&sysmode.1/hercules/&PROGRAM_ID./program_data;
...
/*		ADJ_ENGINE								   AS SYSTEM_CODE,*/
	...
	%if &cmctn_role_cd. = 1 %then %do ;  /** participant data **/
		...
		DATA _NULL_;
		  SET &GSTP_FLAG._ALL ;
		  FILE "&DALCDCP_LOC./&FILE_NM." delimiter="|" DSD RECFM = V lrecl=1000;   
		  PUT  
			SRC_SYS_CD
			INITIATIVE_ID	
			CLNT_LVL1
	...
	%else %if &cmctn_role_cd. = 2 %then %do ;  /** physician data **/
		...
		DATA _NULL_;
		  SET &GSTP_FLAG._ALL ; 
		  FILE "&DALCDCP_LOC./&FILE_NM." DSD delimiter="|" RECFM = V lrecl=1000; 
		  PUT  
			NPI				
			INITIATIVE_ID			
			PRESCRIBER_ID			
