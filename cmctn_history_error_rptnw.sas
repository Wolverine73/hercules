...
*SASDOC************************************************************
* cmctn_history_error_rpt_in.sas - included that logic here :)
*******************************************************************;
OPTIONS NOSYNTAXCHECK;
OPTIONS MISSING='';
OPTIONS COMPRESS=NO;

/*			%LET FTP_user=yourqcpid;*/
/*			%LET FTP_pass=yourqcpid;*/

			**%LET Primary_programer='PRODSUP';
			%LET Primary_programer='QCPI09G';
			**%LET Primary_programer_MF="QCPI254","QCPU570","QCPI115";
			%LET Primary_programer_MF="QCPI09G";
...
PROC FORMAT;
  ...
  '0013' = 'INVALID MULIPLE ROWS FOR CANCEL'
  '0014' = 'INVALID NOT FOUND ON CANCEL'
  '0015' = 'INVALID NPI ID'
  OTHER  = 'UNKNOWN ERROR'
  ;
  VALUE $APPL
...
*******************************************************************;
  ...
  RUN;
	...
	filename RPTFILE "/DATA/sas&sysmode.1/hercules/gen_utilities/sas/update_cmctn/&APPLICATION._&MTH.&DAY.&YR..PDF";
...
*******************************************************************;
	...
	TITLE1 "Weekly Communication History";
	TITLE2 'Transaction Error Reporting';
	TITLE3 "Application - &APPLICATION";
	TITLE4 "Week of &WK";
	PROC PRINT DATA=FINAL_ERR split='*' NOOBS;
	  BY PGM_ID ERROR_NBR APN_CMM_ID;
	  VAR EMAIL_ADDR CMM_GEN_TS RVR_ROLE_CD RVR_ID SBJ_ROLE_CD SBJ_ID DRUG_NDC;
	  FORMAT ERROR_NBR $ERR_CD.; 
	RUN;
	ODS PDF CLOSE;
	ODS LISTING;
...
FILENAME ch_err1 FTP "'QCPPNP.CMA.CMAEB028.ERRORS.FILE(0)'" lrecl=128 host='mainframe.psd.caremark.int'
   USER="&FTP_user" PASS="&FTP_pass" rcmd='site rdw recfm=fb lrecl=128' DEBUG;
FILENAME ch_err2 FTP "'QCPPNP.CMA.CMAEB028.ERRORS.FILE(0)'" lrecl=128 host='mainframe.psd.caremark.int'
   USER="&FTP_user" PASS="&FTP_pass" rcmd='site rdw recfm=fb lrecl=128' DEBUG;
FILENAME ch_err3 FTP "'QCPPNP.CMA.CMAEB028.ERRORS.FILE(0)'" lrecl=128 host='mainframe.psd.caremark.int'
   USER="&FTP_user" PASS="&FTP_pass" rcmd='site rdw recfm=fb lrecl=128' DEBUG;
FILENAME ch_err4 FTP "'QCPPNP.CMA.CMAEB028.ERRORS.FILE(0)'" lrecl=128 host='mainframe.psd.caremark.int'
   USER="&FTP_user" PASS="&FTP_pass" rcmd='site rdw recfm=fb lrecl=128' DEBUG;
FILENAME ch_err5 FTP "'QCPPNP.CMA.CMAEB028.ERRORS.FILE(0)'" lrecl=128 host='mainframe.psd.caremark.int'
   USER="&FTP_user" PASS="&FTP_pass" rcmd='site rdw recfm=fb lrecl=128' DEBUG;
