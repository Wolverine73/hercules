 ...
 *SASDOC -----------------------------------------------------------------------
  ...
  %INCLUDE "/PRG/sastest1/hercules/manual_ins_ovr.sas";
			...
			PROC SQL NOPRINT;
		        ...
		        SELECT * FROM CONNECTION TO DB2
			 	...
			 	WHERE 	A.BENEFACTOR_CLT_ID = B.CLIENT_ID AND
			       		UPPER(A.PB_LISTING_NM) = UPPER(B.PLAN_NM)
...
*SASDOC -----------------------------------------------------------------------------------
  ...
  +--------------------------------------------------------------------------------SASDOC*;
	...
	%*SASDOC ----------------------------------------------------------------------------
 	| NOTE: THIS CHECK WAS ADDED SO THAT THE CHCEK DOCUMENT CAN ALSO RUN FOR
	...
	%*SASDOC ----------------------------------------------------------------------------
 	| Validate if client_level_1 variable exist within dataset
 	+------------------------------------------------------------------------------SASDOC;
	PROC SQL;
	...
	%IF &CLIENTVARIABLE. > 0 AND &MODMAIL. NE %THEN %DO;
		...
		%LET RX_CONS =  %STR(AND B.CARRIER_ID = A.CLIENT_LEVEL_1
            				AND (B.ACCOUNT_ID = ' ' OR 
		...
		%IF %SYSFUNC(EXIST(DATA_PND.&TABLE_PREFIX._&&CMCTN_ROLE_CD&I.)) %THEN %DO;
