...
run;
/*SANJESH : HARDCODING CMCTN_ROLE_CODE*/
%LET CMCTN_ROLE_CD= 1;

options symbolgen;
%macro cee_release_data(init_id=&initiative_id,
                    phase_id=&phase_seq_nb,
                    com_cd=&cmctn_role_cd);

/*SANJESH :CHECKING THE VALUES*/
%PUT NOTE: ROCK  IN CEE_RELEASE DATE ;
%PUT NOTE: ROCK INIT_ID &INIT_ID	;
%PUT NOTE: ROCK phase_id &phase_id	;
%PUT NOTE: ROCK cmctn_role_cd &cmctn_role_cd	;
...
/*		   		%reset_sql_err_cd;*/
	    ...
	    %put NOTE:(&sysmacroname): Task output will be directory to %upcase(&ftp_host):&destination_sub_dir..;
	      ...
	      %if &_destination_cd^=2 %then %do;
                   ...
                   %let file_name_main=/herc&sysmode/hercules/rtml_t_&init_id._&phase_id._&com_cd._&file_id;
		     	  %end;
		     	  %else %do;
                   %let file_name_main=/herc&sysmode/hercules/t_&init_id._&phase_id._&com_cd._&file_id;
		     	  %end;
              ...
              %else  %do;  
	           %***** All Zeus-bound files will be SAS datasets. *****;
		          ...
		          from work.final
	                ...
	                %if &program_id=73 %then %do;
                          %let file_name_main=herc&sysmode/rtml_t_&init_id._&phase_id._&com_cd._&file_id;
	                %end;
	                %else %do;
                          %let file_name_main=herc&sysmode/t_&init_id._&phase_id._&com_cd._&file_id;
	                %end;
...
/*                         letters_sent_qy = (select sum(case data_quality_cd when (1,2) then letters else 0 end)*/
              ...
              %let RPT_NM=/herc&sysmode/report_doc/hercules/general/&init_id._initiative_results_overview.pdf;
		...
		%IF &PROGRAM_ID = 5286 AND &TASK_ID = 33 AND &RELEASE_STATUS_CD = 2 %THEN %DO;
			...
			%macro update_edw_mailing_history;
				...
				**-----------------------------------------------------------------------;
				...
				/*SANJESH :CHECKING VALUES*/
				%PUT NOTE: ROCK  TABLE_PERFIX  &TABLE_PERFIX ;
