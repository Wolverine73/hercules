    ...
    IF INITIATIVE_ID >= 100000	THEN DO;
		...
		IF PROGRAM_ID=30 THEN DO;
			REPORTS_STR=  '~REQUEST_ID=' ||  COMPRESS(INITIATIVE_ID)
				 ...
				 || '~PHASE_SEQ_NB=' || COMPRESS(PHASE_SEQ_NB)
...
/*				 || '~PHASE_SEQ_NB=' || COMPRESS(PARMTR_ID2) */
		...
		ELSE IF PROGRAM_ID=1 THEN DO;								/*	AK - ADDED FOR CLIENT_MAILINGS REPORT 03/31/2013*/
			REPORTS_STR=  '~REQUEST_ID=' ||  COMPRESS(INITIATIVE_ID)
				|| '~REPORT_ID=' || COMPRESS(PROGRAM_ID) 
				|| '~PHASE_SEQ_NB=' || COMPRESS(PHASE_SEQ_NB) 
				|| '~HSC_USR_ID=' || COMPRESS(HSC_USR_ID) ;										
...
%put note: REQUEST_ID=&REQUEST_ID. REPORT_ID=&REPORT_ID. PHASE_SEQ_NB=&PHASE_SEQ_NB. INITIATIVE_ID=&INITIATIVE_ID. HSC_USR_ID=&HSC_USR_ID. ;
