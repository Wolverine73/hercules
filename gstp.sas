...
PROC SQL;
  ...
  SELECT %bquote(/)%bquote(*)+ordered use_hash(claim) full(claim) use_hash(algn) full(algn) use_hash(prctr) full(prctr) use_hash(phmcy) full(phmcy) use_nl(mbr) index(mbr x_mbr_n17)%bquote(*)%bquote(/) 
      CASE
         ...
         FLOOR (MONTHS_BETWEEN (CLAIM.DSPND_DATE, CLAIM.PTNT_BRTH_DT) / 12)
            AS AGE,
         ...
         CASE
            WHEN DRUG.RECAP_GNRC_FLAG = '2'
                 OR DRUG.MDDB_MS_SS_CD IN ('M', 'O', 'N')
            THEN
               'B'
            ELSE
               'G'
...
/*Backup before client connect manipulation*/

	DATA OUT.CLAIMS_B4_CCQL;
	SET CLAIMS;
	RUN;

/*RG CHANGES to pull client_level_1, RPT_MBR_ID */
	PROC SQL;
	CREATE TABLE OTHER_CLAIMS AS
	SELECT DISTINCT CLAIM.*
	FROM CLAIMS CLAIM
	LEFT JOIN DSS_CLIN.V_CLNT_CAG_MGRTN MIGR
	ON LEFT(TRIM(CLAIM.CLIENT_LEVEL_1))  =  LEFT(TRIM(MIGR.SRC_HIER_ALGN_1_ID))
	WHERE LEFT(TRIM(MIGR.SRC_HIER_ALGN_1_ID)) IS NULL OR MIGR.SRC_ADJD_CD NE 'Q'; /*AK added */
	QUIT;

	

	PROC SQL;
	CREATE TABLE CLAIMS_QL_MIGR_A (RENAME=(CLIENT_LEVEL_1 = SRC_CLIENT_LEVEL_1
		TRGT_CLIENT_LEVEL_1 = CLIENT_LEVEL_1))AS
	SELECT DISTINCT CLAIM.*
		  ,MIGR.TRGT_HIER_ALGN_1_ID AS TRGT_CLIENT_LEVEL_1 FORMAT = $22.
	FROM CLAIMS CLAIM 
	INNER JOIN &DSS_CLIN..V_CLNT_CAG_MGRTN MIGR
	ON LEFT(TRIM(CLAIM.CLIENT_LEVEL_1))  =  LEFT(TRIM(MIGR.SRC_HIER_ALGN_1_ID))
	AND MIGR.SRC_ADJD_CD = 'Q'
    AND DATEPART(MIGR.MGRTN_EFF_DT) LE TODAY()
	;
	QUIT;
    
	%LET COUNT_MIGR = &SQLOBS;

	%MACRO migr_clients;
	%IF &COUNT_MIGR > 0 %THEN %DO;
	%PUT COUNT_MIGR = &SQLOBS;
	
	%DROP_ORACLE_TABLE (TBL_NAME=&ORA_TMP..CLAIMS_QL_MIGR_A);
	
	DATA &ORA_TMP..CLAIMS_QL_MIGR_A;
	SET CLAIMS_QL_MIGR_A (RENAME=(MBR=SRC_MBR));
	RUN;
								
	PROC SQL;
	CONNECT TO ORACLE (PATH=&GOLD preserve_comments);
	CREATE TABLE MIGR_CLAIMS_A AS
	SELECT * FROM CONNECTION TO ORACLE
	(SELECT %bquote(/)%bquote(*)+ordered use_hash(mbr) full(mbr)%bquote(*)%bquote(/)
 			QL_MIGR.*
		   ,MBR.RPT_MBR_ID AS MBR
	FROM &ORA_TMP..CLAIMS_QL_MIGR_A QL_MIGR 
		LEFT JOIN &DSS_CLIN..V_MBR MBR
		ON MBR.QL_BNFCY_ID = QL_MIGR.QL_BNFCY_ID
	...
	PROC SQL NOPRINT;
	SELECT COUNT(*) INTO: OTHER_CLAIMS_COUNT FROM OTHER_CLAIMS;
	QUIT;

	%IF &OTHER_CLAIMS_COUNT > 0 %THEN %DO;

	DATA CLAIMS;
	SET OTHER_CLAIMS MIGR_CLAIMS_A;
	RUN;
	%END;
	%ELSE %IF &OTHER_CLAIMS_COUNT = 0 %THEN %DO;
...
,   CLIENT_LEVEL_2    CHAR(22)
,   CLIENT_LEVEL_3    CHAR(22)
