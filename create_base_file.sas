...
| ALLOW MEMBERS WITH INCORRECT PT_BENEFICIARY_ID FOR RECAP AND RXCLAIM TO BE
| PASSED ONTO OUTPUT MEMBER AND PRESCRIBER FILES FOR TARGET BY DRUG AND DSA/NCQA PROGRAMS
+-----------------------------------------------------------------------HEADER*/
%MACRO CREATE_BASE_FILE(TBL_NAME_IN = ,
                        TBL_NAME_IN_LIST = );
...
|STR_FROM2 AND STR_WHERE2 IS NOW BEING USED TO CONSTRUCT A QUERY FOR SCENARIOS OF MISSING OR
| INVALID PT_BENEFICIARY_ID FOR RECAP AND RXCLAIM CLIENTS
+------------------------------------------------------------------------SASDOC*;
...
DATA WORK.ALL_FIELDS;
	...
	LENGTH STR_FIELD $ 500 STR_SELECT $ 5000 STR_SELECT2 $ 100 
		   STR_SELECT3 $ 100 STR_FROM $ 200 STR_FROM2 $ 200 
...
/*		STR_FROM2="FROM &TBL_NAME_IN  AS IN";*/
		STR_FROM='FROM ' || trim("&TBL_NAME_IN.") ||' AS IN';
        STR_FROM2='FROM ' || trim("&TBL_NAME_IN.") ||' AS IN';
	...
	/* Check if RECIPIENT_ID is a participant */
		...
		IF &FILE_SEQ_NB.=1   THEN DO;
			...
			IF IS_RECIPIENT_BENEFICIARY_IN=1 THEN DO;
				STR_FROM2=TRIM(STR_FROM2) || ", &T_DEMOGRAPHICS2  AS TD";
				STR_WHERE2='WHERE IN.MBR_GID = TD.MBR_GID';
			END;
			ELSE DO;
				STR_FROM2=STR_FROM;
    ...
    */
	ELSE IF FIELD_NM='SUBJECT_ID' AND IS_RECIPIENT_BENEFICIARY_IN=0 AND &FILE_SEQ_NB.=1 THEN DO;
		...
		STR_WHERE=TRIM(STR_WHERE) || ' AND IN.'    || TRIM(FIELD_NM_I)
		              || '=' || 'TB.' || TRIM(STR_TMP);
		STR_WHERE2=TRIM(STR_WHERE2) || ' AND IN.MBR_GID = TB.MBR_GID';
	...
	*/
	...
	IF LAST THEN DO;
			...
			CALL SYMPUT('STR_WHERE2',TRIM(STR_WHERE2));
	END;
...
%IF (&CMCTN_ROLE_CD. EQ 1) OR (&PROGRAM_ID EQ 105 
	...
	%IF (&MISSING_CNT. > 0 AND &PROGRAM_ID. ^= 5250) OR (&PROGRAM_ID EQ 105 
    OR (&PROGRAM_ID EQ 106 AND &TASK_ID EQ 21)) %THEN %DO;
		...
		%DROP_DB2_TABLE(tbl_name=&DB2_TMP..CLAIMS_PULL_RES_&INITIATIVE_ID.);
		...
		PROC SQL;
			CREATE TABLE &DB2_TMP..CLAIMS_PULL_RES_&INITIATIVE_ID. AS
			SELECT SUBJECT_ID AS PT_BENEFICIARY_ID
			FROM WORK.&TBL_NAME_OUT_SH.;
		QUIT;
...
*SASDOC--------------------------------------------------------------------------
|JUNE 2012 - TARGET BY DRUG/ DSA AUTOMATION
|EB - BELOW QUERY IS ADDED FOR MEMBERS WITH INCORRECT PT_BENEFICIARY_ID
+------------------------------------------------------------------------SASDOC*;
			CREATE TABLE WORK.&TBL_NAME_OUT_SH.3 AS
			SELECT * FROM CONNECTION TO DB2
			(&STR_SELECT.
			 &STR_FROM2.
			 &STR_WHERE2. and
		   	  ADJ_ENGINE IN ('RX', 'RE')
			  AND IN.PT_BENEFICIARY_ID NOT IN
			  ( SELECT PT_BENEFICIARY_ID
			  FROM &DB2_TMP..CLAIMS_PULL_RES_&INITIATIVE_ID.
			  )
			)
			;
...
*SASDOC--------------------------------------------------------------------------
|JUNE 2012 - TARGET BY DRUG/ DSA AUTOMATION
|EB - FOR PRESCRIBER FILE ONLY SUBJECT_ID NEEDS TO BE SET TO ALL NINES 
+------------------------------------------------------------------------SASDOC*;
...
%END;
		...
		PROC APPEND BASE = WORK.&TBL_NAME_OUT_SH
		            DATA = WORK.&TBL_NAME_OUT_SH.3 FORCE;
		RUN;quit;
...
*SASDOC--------------------------------------------------------------------------
|JUNE 2012 - TARGET BY DRUG/ DSA AUTOMATION
|EB - PER BUSINESS, THE BELOW HAS BEEN COMMENTED OUT
+------------------------------------------------------------------------SASDOC*;
/** PRESCRIBER EXCLUSIONS FOR QUALITY **/
/*%IF &CMCTN_ROLE_CD. EQ 2 %THEN %DO;*/
/*	%PRESCRIBER_OPT_OUT(TBL_NAME_IN=WORK.&TBL_NAME_OUT_SH.,*/
...
%PUT STR_WHERE2 = &STR_WHERE2;

%include "/herc&sysmode/prg/hercules/macros/add_rocc_cd.sas";
/*%include "/PRG/sasprod1/hercules/macros/add_rocc_cd.sas";*/
...
%UPDATE_PRIVACY_ADDRESS(INPUT_TABLE=&TBL_NAME_OUT_SH., ADJUDICATION=RX);	/*	AK - November 2013 - Call update privacy address macro*/
%IF &FILE_SEQ_NB=1 %THEN %DO;
	%IF &FILE_ID. EQ 17 %THEN  %DO;                 
		%GET_DATA_QUALITY(TBL_NAME_IN = WORK.&TBL_NAME_OUT_SH.,
				  TBL_NAME_OUT=DATA_PND.&TBL_NAME_OUT_SH.);
	%END;
	%ELSE %IF (&FILE_ID. EQ 32) OR (&CMCTN_ROLE_CD EQ 1 AND (&PROGRAM_ID EQ 105 
    OR (&PROGRAM_ID EQ 106 AND &TASK_ID EQ 21))) %THEN %DO;
	   		%include "/herc&sysmode/prg/hercules/macros/additional_fields.sas";
/*	   		%include "/PRG/sasprod1/hercules/macros/additional_fields.sas";*/
	  		%GET_DATA_QUALITY_GSTP(TBL_NAME_IN = WORK.&TBL_NAME_OUT_SH.,
	  				  TBL_NAME_OUT=DATA_PND.&TBL_NAME_OUT_SH.);
	%END;
	%ELSE %DO;
 		%include "/herc&sysmode/prg/hercules/macros/additional_fields.sas";
