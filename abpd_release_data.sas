...
options symbolgen;
%macro abpd_release_data(init_id=&initiative_id,
                    phase_id=&phase_seq_nb,
                    com_cd=&cmctn_role_cd);

/*SANJESH :CHECKING THE VALUES*/
%PUT NOTE: CHECK IN ABPD_RELEASE DATE ;
%PUT NOTE: CHECK INIT_ID &INIT_ID	;
%PUT NOTE: CHECK phase_id &phase_id	;
%PUT NOTE: CHECK cmctn_role_cd &cmctn_role_cd	;
		...
		%IF &PROGRAM_ID = 5286 AND &TASK_ID = 33 AND &RELEASE_STATUS_CD = 2 %THEN %DO;
			...
			%macro update_edw_mailing_history;
				...
				**-----------------------------------------------------------------------;
				...
				%PUT NOTE: CHECK  TABLE_PERFIX  &TABLE_PERFIX ;
