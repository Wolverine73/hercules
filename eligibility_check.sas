...
|
|22MAR2013 - M.BEEZHOLD - ITPR004354 (Exposed TBD PII-key joining to all programs and tasks)
|
|05SEP2013 - R.PILECKIS - Logic change for BSR Client Connect Recap RxClaim Eligibility Fix
|
----------------------------------------------------------------------------------------HEADER*/
...
/* New RP */
				%IF &CC_RE_MIGR_IND. = 1 and NOT (&PROGRAM_ID EQ 105 OR (&PROGRAM_ID EQ 106 AND 
				&TASK_ID EQ 21))%THEN %DO;
/* New RP */
...
%THEN %DO;
				PROC SQL NOPRINT;
	  			...
	  			FROM &ORA_TMP..ALGN_LVL_LIST_&INITIATIVE_ID._&adj2.;
				QUIT;
%END;
%ELSE %DO;
	%LET LVL1_CNT = 0;
...
*SASDOC--------------------------------------------------------------------------
|MB, MARCH 2013 - ITPR004354: Added to enable dynamic generation of column-references
|depending on column-presence in input-tables.
|Missing PII-key columns will be added here.
+------------------------------------------------------------------------SASDOC*;

%let TABLENAME1=%sysfunc(tranwrd(&TBL_NAME_IN_EDW,&ORA_TMP..,)); /*Strip schema from the table-name.*/
%let TABLENAME2=%sysfunc(compress(&TABLENAME1,_)); /*Avoids exceeding Oracle's maximum name-length.*/
%put NOTE: TABLENAME1=&TABLENAME1;
%put NOTE: TABLENAME2=&TABLENAME2;

%drop_oracle_table(TBL_NAME=&ORA_TMP..&TABLENAME2); /*Give it time to operate.*/

%local PII0;
%let PII0=0;

%macro Loop(START,STOP,STMT);
  %local N;
  %do N=&START %to &STOP;
    %unquote(&STMT)
  %end;
...
proc datasets nolist;
  contents
  noprint
  data=&TBL_NAME_IN_EDW
  out=WORK.PIIKEY_FOUND (
  ...
  noprint
  data=&TBL_NAME_IN_EDW
  out=WORK.PIIKEY_JOINCOLUMNS (
    keep=NAME
    where=(NAME in('MBR_GID','PAYER_ID','LAST_FILL_DT'))
  )
  ;
quit;

data _NULL_;
  set WORK.PIIKEY_FOUND end=DONE;
  call symputx(cats('PII',_N_),NAME);
  if(NAME^='M_DOB')
  then call symputx(cats('PIIALT',_N_),NAME);
  else call symputx(cats('PIIALT',_N_),'MBR_BRTH_DT');
  if (DONE) then call symputx('PII0',_N_);
run;

%Loop(1,&PII0,%nrstr(%put NOTE: PII&N="&&PII&N";))

data WORK.PIIKEY_EXPECTED;
  length NAME $32;
  NAME='M_DOB'; output;
  NAME='MBR_FIRST_NM'; output;
  NAME='MBR_GNDR_GID'; output;
  NAME='REL_CODE'; output;
  NAME='SRC_SUFFX_PRSN_CD'; output;
run;

proc sql noprint;
  create table WORK.PIIKEY_NEEDED as
  select A.NAME
  from   WORK.PIIKEY_EXPECTED as A
  left
  join   WORK.PIIKEY_FOUND as B
  on     A.NAME=B.NAME
  where  B.NAME is null
  ;
  select count(*) into :NEEDED
  from   WORK.PIIKEY_NEEDED
  ;
  select count(*) into :JOINCOLUMNS
  from   WORK.PIIKEY_JOINCOLUMNS
  ;
quit;

%if &NEEDED>0 & &JOINCOLUMNS=3 %then %do; /*<-- Start of adding missing PII-key columns.*/

data _NULL_;
  set WORK.PIIKEY_NEEDED end=DONE;
  call symputx(cats('PII',_N_),NAME);
  if(NAME^='M_DOB')
  then call symputx(cats('PIIALT',_N_),NAME);
  else call symputx(cats('PIIALT',_N_),'MBR_BRTH_DT');
...
%put NOTE: &TBL_NAME_IN_EDW - &EDW_CLAIM_CNT;
%if &EDW_CLAIM_CNT >= 25000000 
%then	%let AVOID_IX_LKP=%str(/1);
%else %let AVOID_IX_LKP=%str();

%if &ADJ2=RE %then %let PAYER_ID_CONS=%str(BETWEEN 500000 AND 2000000);
%else
%if &ADJ2=RX %then %let PAYER_ID_CONS=%str(< 100000);

proc sql noprint;
  connect to Oracle(path=&GOLD PRESERVE_COMMENTS);
  execute(rename &TABLENAME1 to &TABLENAME2) by Oracle;
  execute(
    create table &TBL_NAME_IN_EDW as
    select CLAIM.*
    %Loop(1,&PII0,%nrstr(,MBR.&&PIIALT&N as &&PII&N))
    from  &ORA_TMP..&TABLENAME2 CLAIM     
    ,     &DSS_CLIN..V_MBR MBR
    where MBR.PAYER_ID&AVOID_IX_LKP. &PAYER_ID_CONS.
    and   CLAIM.PAYER_ID&AVOID_IX_LKP. = MBR.PAYER_ID&AVOID_IX_LKP.
    and   CLAIM.MBR_GID&AVOID_IX_LKP. = MBR.MBR_GID&AVOID_IX_LKP.
    and (
    (MBR.MBR_REUSE_RISK_FLG IS NULL)
    or
...
data _NULL_;
  set WORK.PIIKEY_ADDED end=DONE;
  call symputx(cats('PII',_N_),NAME);
  if(NAME^='M_DOB')
  then call symputx(cats('PIIALT',_N_),NAME);
  else call symputx(cats('PIIALT',_N_),'MBR_BRTH_DT');
  if (DONE) then call symputx('PII0',_N_);
run;
%end; /*<-- End of adding missing PII-key columns.*/
...
|MARCH 2013 - ITPR004354
|MB - Extended use of additional matching-columns, referenced above, to all RE/RX programs and tasks.
|MB - Added MBR_FIRST_NM and SRC_SUFFX_PRSN_CD to matching-columns list.
+------------------------------------------------------------------------SASDOC*;
     proc sql;
	  ...
	  SELECT  A.mbr_id,
          ...
          FROM  &ora_tmp..PTSa&adj2.&INIT_ID. A 	                   	 
            ...
            %END;
				 ...
				 &ELIG_STREDW_ALGN.
         %Loop(1,&PII0,%nrstr(and (A.&&PII&N=D.&&PIIALT&N or (A.&&PII&N is null or D.&&PIIALT&N is null))))
...
/*Added sort-procedure as order-by in Oracle-query was not recognized in the following data-step.*/

proc sort data=WORK.PTSB&ADJ2.&INIT_ID tagsort;
  by
  MBR_ID
  PAYER_ID
  %Loop(1,&PII0,%nrstr(&&PII&N))
  descending ELIG_END_DT
  descending ELIG_EFF_DT
  ;
run;
      ...
      (
	    ...
	    SELECT A.*
			...
			%IF (&PROGRAM_ID EQ 105 OR (&PROGRAM_ID EQ 106 AND &TASK_ID EQ 21)) %THEN %DO;
							...
							/*,D.MBR_FIRST_NM <----- Now part of PII-key.*/
							,D.MBR_LAST_NM				
							,D.ADDR_LINE1_TXT			
							,D.ADDR_LINE2_TXT			
							,D.ADDR_CITY_NM				
		...
		/*%Loop(1,&PII0,%nrstr(and A.&&PII&N=B.&&PII&N))*/
         %Loop(1,&PII0,%nrstr(and (A.&&PII&N=B.&&PII&N	  or (A.&&PII&N is null or B.&&PII&N is null   ))))		/*AK added null value conditions - Jan2014*/
			%IF (&PROGRAM_ID EQ 105 OR (&PROGRAM_ID EQ 106 AND &TASK_ID EQ 21)) %THEN %DO;
			 AND B.MBR_GID = D.MBR_GID
			 and D.SRC_SYS_CD = &ADJ
