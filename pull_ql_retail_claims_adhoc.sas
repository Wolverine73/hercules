...
options mprint mprintnest mlogicnest mlogic source2 symbolgen;
   ...
   CONNECT TO DB2 AS DB2(DSN=&UDBSPRP);
     ...
     EXECUTE(create TABLE &tbl_name_out.	
                 (PT_BENEFICIARY_ID INTEGER NOT NULL,
				  ...
				  NABP_ID CHAR(12),
				  DAY_SUPPLY_QY SMALLINT NOT NULL) NOT LOGGED INITIALLY) BY DB2;
   DISCONNECT FROM DB2;
 QUIT;
    ...
    CONNECT TO DB2 AS DB2(DSN=&UDBSPRP AUTOCOMMIT=NO);
     ...
     EXECUTE(INSERT INTO &tbl_name_out.
             SELECT
                     ...
                     C.DRUG_ABBR_PROD_NM,
                     B.DRUG_CATEGORY_ID,
					 A.NABP_ID AS NABP_ID,
					 A.DAY_SUPPLY_QY as DAY_SUPPLY_QY

           FROM    &claimsa..&claim_his_tbl A,
                   &tbl_name_in. B,
                   &claimsa..TDRUG1 C
 				WHERE  A.FILL_DT BETWEEN '2011-09-01' AND '2012-02-29' 
