...
%drop_db2_table(tbl_name=&tbl_name_out._1);
 PROC SQL;
   CONNECT TO DB2 AS DB2(DSN=&UDBSPRP);
   EXECUTE(CREATE TABLE &tbl_name_out._1 AS
 ...
 PROC SQL;
   ...
   EXECUTE(CREATE TABLE &tbl_name_out. AS
	 			(SELECT 	 A.PT_BENEFICIARY_ID ,
								 ...
								 A.REFILL_FILL_QY,
								 A.FORMULARY_TX,
								 A.GENERIC_NDC_IN,/* NEW FIELDS FROM TDRUG */
								 A.DRUG_ABBR_STRG_NM,
								 A.DRUG_ABBR_DSG_NM,
								 A.CLIENT_LEVEL_1 ,
							     A.LAST_DELIVERY_SYS,
...
PROC SQL;
    CONNECT TO DB2 AS DB2(DSN=&UDBSPRP AUTOCOMMIT=NO);
     ...
     EXECUTE(INSERT INTO &tbl_name_out.
		...
		WHERE  A.CLT_PLAN_GROUP_ID2 = H.CLT_PLAN_GROUP_ID  /* Newly added CONDITION FOR JOINING TABLES */
