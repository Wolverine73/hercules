/*HEADER---------------------------------------------------------------------------------------------------------
|MACRO: 		CLAIMS_PULL_EDW_PROACTIVE.SAS
|
...
|
|JAN2014 - A.Kolakotla - Added proactive BSR changes for RECAP 0 claims condition
+-----------------------------------------------------------------------------------------------------------HEADER*/
...
/*%MACRO CLAIMS_PULL_EDW_TST(DRUG_NDC_TABLE_RX = &ORA_TMP..&TABLE_PREFIX._NDC_RX,*/
...
+------------------------------------------------------------------------SASDOC*;
%MACRO CLAIMS_PULL_EDW_PROACTIVE(DRUG_NDC_TABLE_RX = &ORA_TMP..&TABLE_PREFIX._NDC_RX,
					   DRUG_NDC_TABLE_RE = &ORA_TMP..&TABLE_PREFIX._NDC_RE, 
					   DRUG_RVW_DATES_TABLE = &ORA_TMP..&TABLE_PREFIX._RVW_DATES,
					   RESOLVE_CLIENT_TABLE_RX = &ORA_TMP..&TABLE_PREFIX._RX ,
					   RESOLVE_CLIENT_TABLE_RE = &ORA_TMP..&TABLE_PREFIX._RE
...
/*      RUN;*/
		...
		%PUT NOTE: CLIENT SPECIFIC DRUG LIST DATE RANGES;
...
/*               and clt.client_level_1=4740*/
/*               AND CLT.CLIENT_LEVEL_1<>'GYY'*/
/*               AND CLT.CLIENT_LEVEL_1<>'D32' */
/*               AND CLT.CLIENT_LEVEL_1<>'GFW' */
/*               AND CLT.CLIENT_LEVEL_1<>'19021' */
/*               AND CLT.CLIENT_LEVEL_1<>'D69'*/
			   ...
			   AND CLT.CLIENT_LEVEL_1 NOT IN (&&CLIENTS_&ADJ_ENGINE.)		/*AK added condition*/
...
/*YM:Nov12,2012:ADDED DAY_SUPPLY_QY FOR PROACTIVE REFILL(7.8)*/
		PROC SQL;
  			CONNECT TO ORACLE(PATH=&GOLD PRESERVE_COMMENTS);
  			CREATE TABLE data_res.mail_claims_&adj_engine._&initiative_id. AS
  				SELECT * FROM CONNECTION TO ORACLE
  				( 
          SELECT  	%BQUOTE('&ADJ_ENGINE') AS ADJ_ENGINE,
                    clt.algn_lvl_gid_key,
...
/*               AND CLT.CLIENT_LEVEL_1<>'GYY'*/
               ...
               AND CLAIM.SRC_SYS_CD = %BQUOTE('&SRC_SYS_CD')
               AND claim.claim_wshd_cd IN ('P', 'W')
			   AND CLT.CLIENT_LEVEL_1 NOT IN (&&CLIENTS_&ADJ_ENGINE.)				
  /*             AND (claim.mbr_suffx_flg = 'Y' OR claim.mbr_suffx_flg IS NULL)*/
  /*             AND claim.ql_void_ind <= 0*/
          );
          DISCONNECT FROM ORACLE;
...
/*	JAN2014 - BR2 FR2 DD.002*/
%LET ERR_FL = 0;	/*AK SWITCHED OFF ERROR CONDITION - PROACTIVE BSR - JAN2014*/

%ON_ERROR(ACTION=CONTINUE
  	          ,EM_TO=&PRIMARY_PROGRAMMER_EMAIL
  	          ,EM_SUBJECT=HCE SUPPORT: NOTIFICATION OF ABEND INITIATIVE_ID &INITIATIVE_ID
  	          ,EM_MSG=%STR(CLAIMS_PULL_EDW MACRO RETURNED 0 ROWS FOR ADJ &ADJ_ENGINE. 
								BUT THE PROGRAM WILL CONTINUE WITH DUMMY DATA FOR OTHER PARTS OF THE PROCESS TO RUN));	/*AK CHANGED MESSAGE - JAN2014*/


/*	AK ADDED DUMMY TABLE TO ACCOUNT FOR 0 CLAIMS CONDITION	*/
PROC SQL; 
CONNECT TO ORACLE (PATH=&GOLD);
	EXECUTE (
				CREATE TABLE &ORA_TMP..CLAIMS_PULL_&INITIATIVE_ID._&ADJ_ENGINE.
				(
				  ADJ_ENGINE          VARCHAR2(2 BYTE),
				  ALGN_LVL_GID_KEY    NUMBER(14),
				  CLIENT_ID           NUMBER,
				  CLIENT_NM           VARCHAR2(60 BYTE),
				  CLIENT_LEVEL_1      VARCHAR2(22 BYTE),
				  CLIENT_LEVEL_2      VARCHAR2(22 BYTE),
				  CLIENT_LEVEL_3      VARCHAR2(22 BYTE),
				  PAYER_ID            NUMBER,
				  MBR_GID             NUMBER,
				  DRUG_GID            NUMBER,
				  PHMCY_GID           NUMBER,
				  ...
				  BNFT_LVL_CODE       VARCHAR2(10 BYTE),
				  BIRTH_DT            VARCHAR2(10 BYTE),
				  LAST_FILL_DT        VARCHAR2(10 BYTE),
				  RX_COUNT_QY         NUMBER,
				  REFILL_FILL_QY      NUMBER,
				  LTR_RULE_SEQ_NB     NUMBER,
				  RX_NB               VARCHAR2(20 BYTE),
				  DISPENSED_QY        NUMBER,
				  DAY_SUPPLY_QY       VARCHAR2(4 BYTE),
				  FRMLY_GID           NUMBER,
				  DRUG_NDC_ID         VARCHAR2(20 BYTE),
				  DRUG_CATEGORY_ID    NUMBER,
				  GPI14               VARCHAR2(19 BYTE),
				  DRUG_BRAND_CD       VARCHAR2(1 BYTE),
				  DRUG_ABBR_DSG_NM    VARCHAR2(30 BYTE),
				  DRUG_ABBR_PROD_NM   VARCHAR2(30 BYTE),
				  DRUG_ABBR_STRG_NM   VARCHAR2(30 BYTE),
				  GCN_CODE            NUMBER(7),
				  GCN_NBR             NUMBER(6),
				  BRAND_GENERIC       VARCHAR2(1 BYTE),
				  NABP_CODE_6         VARCHAR2(15 BYTE),
				  PHARMACY_NM         VARCHAR2(60 BYTE),
				  LAST_DELIVERY_SYS   NUMBER,
				  DELIVERY_SYSTEM     VARCHAR2(30 BYTE),
				  PRACTITIONER_ID     VARCHAR2(20 BYTE),
				  PRESCRIBER_ID       VARCHAR2(11 BYTE),
				  ENTITY_IND          CHAR(1 BYTE),
				  DEGR_1_CD           CHAR(3 BYTE),
				  PRESCRIBER_NPI_NB   VARCHAR2(20 BYTE),
				  DEA_NB              VARCHAR2(20 BYTE),
				  PT_BENEFICIARY_ID   NUMBER,
				  CDH_BENEFICIARY_ID  VARCHAR2(20 BYTE),
				  MBR_ID              VARCHAR2(25 BYTE),
				  MBR_FIRST_NM        VARCHAR2(40 BYTE),
				  MBR_LAST_NM         VARCHAR2(40 BYTE),
				  ADDR_LINE1_TXT      VARCHAR2(60 BYTE),
				  ADDR_LINE2_TXT      VARCHAR2(60 BYTE),
				  ADDR_CITY_NM        VARCHAR2(60 BYTE),
				  ADDR_ST_CD          VARCHAR2(3 BYTE),
				  ADDR_ZIP_CD         VARCHAR2(20 BYTE),
				  SRC_SUFFX_PRSN_CD   VARCHAR2(3 BYTE),
				  ALT_INS_MBR_ID      VARCHAR2(25 BYTE),
				  FORMULARY_TX        VARCHAR2(10 BYTE),
				  M_DOB               DATE,
				  MBR_GNDR_GID        NUMBER,
				  REL_CODE            VARCHAR2(2 BYTE)
				)
)BY ORACLE;
...
/*	END AK ADDED - JAN2014 - PROACTIVE REFILL BSR	*/
  ...
  *SASDOC--------------------------------------------------------------------------
  | 08NOV2012 - Hercules Version  2.1.2.02
  ...
  /*		%DROP_ORACLE_TABLE(TBL_NAME = &ORA_TMP..ALGN_LVL_LIST_&INITIATIVE_ID._&ADJ_ENGINE.);*/
	...
	%IF &RX_ADJ. = 1 AND &ADJ_ENGINE. = RX  %THEN %DO;
	...
	%IF &RE_ADJ. = 1  AND &ADJ_ENGINE. = RE  %THEN %DO;
...
%MEND CLAIMS_PULL_EDW_PROACTIVE;
