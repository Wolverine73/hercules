...
%MACRO test(EM_TO=,EM_CC=,EM_SUBJECT=,EM_MSG=,EM_ATTACH=);
...
%LET err_fl=0;
       ...
       %LET DB2_TMP0 = %UPCASE(&USER);
%LET EMAILTO = "sameer.shrivastava@caremark.com";
%PUT &EMAILTO;

       %email_parms(EM_TO=&EMAILTO,
                     EM_SUBJECT="SUCCESS: daily_benef_xref_dn_load.sas program for &todd completed successfully.",
                       EM_MSG="The Daily Denormalized Beneficiary CrossReference Load program has completed successfully.");
...
%MACRO email(EM_TO=,EM_CC=,EM_SUBJECT=,EM_MSG=,EM_ATTACH=);
/*OPTIONS NOSYNTAXCHECK;*/
%GLOBAL Err_cd Message;
